...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin, CLIBillingMixin):
    ...
    def run(self):
        ...
        # never sends ESC[6n cursor-position queries on terminals where delayed
        # replies can leak into the display or stall input (#13870; local POSIX
        # under heavy subagent/status-line load). The selector keeps native
        # Windows on prompt_toolkit's default output, and returns None on local
        # preserve-default or construction failure so Application falls back
        # safely; input-side scrubbing still guards residual leaks.
...
def main(
    ...
    if toolsets:
        ...
        # workspace. See agent/coding_context.py. If coding context cannot be
        # resolved, fall back to the shared resolver so configured MCP servers
        # still participate at runtime.
        _coding = None
        try:
