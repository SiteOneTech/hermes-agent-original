...
def test_claim_next_task_allows_reporter_g1_recovery_terminal_report_language_past_validation_readiness(
    fake_sql, monkeypatch
):
    documentation_recovery = {
        "project_id": "demo",
        "lane_id": "lane-g1-report",
        "task_id": "demo-r2d3-terminal-report-g1-recovery",
        "status": "todo",
        "phase": "g1_recovery",
        "priority": 17,
        "title": "R2d3 — current-source G1 validation-preflight terminal-report recovery",
        "description": (
            "Final report handoff wording is historical provenance only. Recover required G1 "
            "documentation review evidence from the current source without product, ALR, runtime, "
            "delivery, deploy, direct-SQL, messaging, trading, risk, or paper/live action."
        ),
        "owner_profile": "factory-reporter",
        "reviewer_profile": "quality-reviewer",
        "engine": "codex",
    ...
    delivery_report = {
        **documentation_recovery,
        ...
        "description": "Prepare the terminal delivery report after validation is complete.",
    }
    tasks = [product, documentation_recovery]
    project = {
        "project_id": "demo",
    ...
    monkeypatch.setattr(
        factory_pg,
        "_validation_task_readiness_findings",
        lambda project_id: ["validation task demo-quality-review is not complete; status=ready"],
    )
    ...
    assert factory_pg._dispatch_preflight_blockers(delivery_report, docs_ready=False, notion_ready=True) == [
        "missing_or_unindexed_docs"
    ]
    assert factory_pg._candidate_requires_validation_readiness_before_dispatch(delivery_report) is True
