...
def test_force_tick_routes_structured_documentation_phase_before_review_preflight(fake_sql, monkeypatch):
    product_review = {
        "project_id": "demo",
        "lane_id": "lane-review",
        "task_id": "demo-r2cy-r1-independent-exact-sha-quality-re",
        ...
        "priority": 17,
        "title": "R2cy-R1 — independent exact-SHA quality review",
        "description": "Product quality review must wait for green G1 document rows.",
        "owner_profile": "quality-reviewer",
        "reviewer_profile": "security-reviewer",
        "engine": "codex",
        "dependencies": [],
    ...
    product = {
        ...
        "priority": 18,
        "title": "ALR-020 product implementation",
        "description": "Normal product work remains docs-first gated while G1 rows are red.",
        "owner_profile": "claude-builder",
        "engine": "claude_code",
        "dependencies": [],
        "metadata": {},
    }
    structured_documentation_recovery = {
        "project_id": "demo",
        "lane_id": "lane-docs",
        "task_id": "demo-structured-phase-candidate",
        "status": "todo",
        "phase": "documentation",
        "priority": 19,
        "title": "Structured phase candidate",
        "description": (
            "Final gate closure wording is historical evidence only; select this "
            "same-project Factory control-plane candidate from structured state and phase."
        ),
        "owner_profile": "codex-builder",
        "reviewer_profile": "quality-reviewer",
        "engine": "codex",
    ...
    document_status = [{"file_name": "PRD.md", "category": "g1_required", "blocking": True}]
    project = {
        "project_id": "demo",
        "status": "active",
        "autonomous_enabled": True,
    ...
    tasks = [product_review, product, structured_documentation_recovery]
    ...
    monkeypatch.setattr(
        factory_pg,
        "_validation_task_readiness_findings",
        lambda project_id: ["validation task demo-r2cy-r1-independent-exact-sha-quality-re is not complete; status=review_ready"],
    )
    ...
    def rows(sql_text, *, user=None, **_):
        ...
        if "t.status='review_ready'" in sql_text:
            return [product_review]
        if "status IN ('todo', 'ready')" in sql_text:
            return [product, structured_documentation_recovery]
        return []
    ...
    def statement_one(sql_text, *, user=None, **_):
        ...
        if "SET status='review_running'" in sql_text:
            return {**product_review, "status": "review_running"}
        if "SET status='claimed'" in sql_text and structured_documentation_recovery["task_id"] in sql_text:
            return {**structured_documentation_recovery, "status": "claimed"}
        if "SET status='claimed'" in sql_text and product["task_id"] in sql_text:
            return {**product, "status": "claimed"}
        return None
    ...
    assert tick["claimed"] is not None
    assert tick["claimed"]["task"]["task_id"] == structured_documentation_recovery["task_id"]
    joined = "\n".join(fake_sql.statements)
    assert f"Task {structured_documentation_recovery['task_id']} claimed" in joined
    assert f"Task {product_review['task_id']} claimed for review" not in joined
    assert f"Task {product['task_id']} claimed" not in joined
    assert "dispatch_preflight_denied" not in joined
