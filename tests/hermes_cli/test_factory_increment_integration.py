...
def test_force_tick_routes_structured_independent_review_recovery_before_product_preflight(fake_sql, monkeypatch):
    required_doc_names = [
        "FACTORY_INTAKE.md",
        "REQUIREMENTS_ANALYSIS.md",
        "PATTERN_ANALYSIS.md",
    ...
    product_review = {
        ...
        "description": "Review Alpha Ledger product changes after readiness is green.",
        "owner_profile": "quality-reviewer",
        "reviewer_profile": "security-reviewer",
        "engine": "codex",
        "dependencies": [],
    ...
    product = {
        ...
        "priority": 10,
        "title": "Product implementation",
        "description": "Implement Alpha Ledger product behavior after readiness is green.",
        "owner_profile": "claude-builder",
        "engine": "claude_code",
        "dependencies": [],
        "metadata": {},
    ...
    r2df_r39 = {
        "project_id": "demo",
        "lane_id": "lane-g1",
        "task_id": "demo-r2df-r39-terminalization",
        "status": "todo",
        "phase": "g1_recovery",
        "priority": 11,
        "title": "R2df-R39 control-plane task",
        "description": "Structured metadata makes this unsafe for autonomous dispatch in red G1.",
        "owner_profile": "codex-builder",
        "engine": "codex",
        "dependencies": [],
        "metadata": {"g1_recovery": True, "runtime_scope": "external_runtime"},
    }
    r2df_r17 = {
        "project_id": "demo",
        "lane_id": "lane-g1",
        "task_id": "demo-r2df-r17-validation-scheduler",
        "status": "ready",
        "phase": "g1_recovery",
        "priority": 12,
        "title": "R2df-R17 control-plane task",
        "description": "Structured metadata makes this unsafe for autonomous dispatch in red G1.",
        "owner_profile": "codex-builder",
        "engine": "codex",
        "dependencies": [],
    ...
    r2df_fresh = {
        "project_id": "demo",
        "lane_id": "lane-g1",
        "task_id": "demo-r2df-fresh-documentation",
        "status": "todo",
        "phase": "documentation",
        "priority": 13,
        "title": "R2df current-base task",
        "description": "Structured metadata makes this unsafe for autonomous dispatch in red G1.",
        "owner_profile": "codex-builder",
        "engine": "codex",
        "dependencies": [],
    ...
    independent_review_recovery = {
        ...
        "task_id": "demo-independent-review-candidate",
        "status": "ready",
        "phase": "quality_review",
        "priority": 14,
        "title": "Independent candidate",
        "description": "Bounded same-project candidate with no product, deployment, credential, messaging, trading, risk, or activation scope.",
        "owner_profile": "quality-reviewer",
        "reviewer_profile": "security-reviewer",
        "engine": "codex",
        "dependencies": [],
        "metadata": {
            "independent_review_recovery": True,
            "review_target_phase": "g1_recovery",
            "no_product_runtime_scope": True,
        },
    }
    tasks = [product_review, product, r2df_r39, r2df_r17, r2df_fresh, independent_review_recovery]
    project = {
        "project_id": "demo",
        "status": "active",
        "autonomous_enabled": True,
    ...
    assert factory_pg._dispatch_preflight_blockers(product_review, docs_ready=False, notion_ready=True) == [
        "missing_or_unindexed_docs"
    ]
    for unsafe_recovery in (r2df_r39, r2df_r17, r2df_fresh):
        assert factory_pg._dispatch_preflight_blockers(unsafe_recovery, docs_ready=False, notion_ready=True) == [
            "missing_or_unindexed_docs"
        ]
    ...
    monkeypatch.setattr(
        factory_pg,
        "_validation_task_readiness_findings",
        lambda project_id: ["validation task demo-product-quality-review is not complete; status=ready"],
    )
    ...
    def rows(sql_text, *, user=None, **_):
        ...
        if "t.status='review_ready'" in sql_text:
            return [product_review]
        if "status IN ('todo', 'ready')" in sql_text:
            return [product, r2df_r39, r2df_r17, r2df_fresh, independent_review_recovery]
        return []
    ...
    def statement_one(sql_text, *, user=None, **_):
        ...
        if "SET status='review_running'" in sql_text:
            return {**product_review, "status": "review_running"}
        if "SET status='claimed'" in sql_text and independent_review_recovery["task_id"] in sql_text:
            return {**independent_review_recovery, "status": "claimed"}
        return None
    ...
    assert tick["claimed"] is not None
    assert tick["claimed"]["task"]["task_id"] == independent_review_recovery["task_id"]
    joined = "\n".join(fake_sql.statements)
    assert f"Task {independent_review_recovery['task_id']} claimed" in joined
    assert f"Task {product_review['task_id']} claimed for review" not in joined
    assert f"Task {product['task_id']} claimed" not in joined
    assert f"Task {r2df_r39['task_id']} claimed" not in joined
    assert f"Task {r2df_r17['task_id']} claimed" not in joined
    assert f"Task {r2df_fresh['task_id']} claimed" not in joined
    assert "dispatch_preflight_denied" not in joined
