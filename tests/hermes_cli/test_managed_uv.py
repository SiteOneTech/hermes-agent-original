...
class TestUpdateManagedUv:
    ...
    def test_self_update_success(self, tmp_path):
        ...
        with patch("hermes_cli.managed_uv.get_hermes_home", return_value=tmp_path), \
            ...
            assert result == str(tmp_path / "bin" / "uv")
            # First call is self update, second is --version. Runtime repair is
            # covered separately and isolated here so this test stays focused on
            # the uv self-update command sequence.
