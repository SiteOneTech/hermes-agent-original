...
class TestDeletedProfileTombstone:
    ...
    def test_failed_create_keeps_tombstone(self, profile_env):
        profile_dir = create_profile("worker", no_alias=True, no_skills=True)
        _delete("worker")
    ...
    def test_import_after_delete_clears_target_tombstone(self, profile_env, tmp_path):
        profile_dir = create_profile("worker", no_alias=True, no_skills=True)
        _delete("worker")
        payload = tmp_path / "payload" / "worker"
        payload.mkdir(parents=True)
        (payload / "config.yaml").write_text("model: test\n", encoding="utf-8")
        archive = tmp_path / "worker.tar.gz"
        with tarfile.open(archive, "w:gz") as tf:
            tf.add(payload, arcname="worker")
    ...
    def test_rename_to_deleted_name_clears_target_tombstone(self, profile_env):
        create_profile("worker", no_alias=True, no_skills=True)
        target_dir = create_profile("retired", no_alias=True, no_skills=True)
        _delete("retired")

        with patch("hermes_cli.profiles.check_alias_collision", return_value="skip"):
            renamed = rename_profile("worker", "retired")

        assert renamed == target_dir
        assert not named_profile_is_deleted(target_dir)
        assert profile_exists("retired")
        assert "retired" in _named_homes(profile_env)
