...
class TestSystemUnitHermesHome:
    ...
    def test_system_unit_orders_after_target_user_manager(self, monkeypatch, tmp_path):
        ...
        # Simulate sudo: the CALLING user's home is a root-style dir distinct from the target
        # user's. The user-scope unit walks Path.home()/.local/bin etc. (_build_user_local_paths),
        # so keep it under tmp_path rather than the real /root (EACCES for non-root runners).
        root_home = tmp_path / "root"
        root_home.mkdir()
        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
