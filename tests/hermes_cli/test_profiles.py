...
class TestListProfiles:
    ...
    def test_name_listing_drops_tombstoned_profiles(self, profile_env):
        """A tombstoned directory is not a profile on the cheap name-only listing either.

        ``list_profile_names`` feeds the cron bot-chat target list and kanban's "installed
        profiles" line; a stale process that re-mkdirs a deleted profile's directory must not
        resurface it as a ``bot-chat:<name>`` target.
        """
        from hermes_constants import mark_named_profile_deleted

        create_profile("retiring", no_alias=True)
        mark_named_profile_deleted(get_profile_dir("retiring"))

        assert profiles.profile_exists("retiring") is False
        assert "retiring" not in profiles.list_profile_names()
