...
class TestWebServerEndpoints:
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        data = self.client.get("/api/env").json()
        expected_by_primary: dict[str, object] = {}
        for d in provider_catalog():
            if d.tab != "keys" or not d.api_key_env_vars:
                continue
            # A single env-var row cannot represent two providers. Providers
            # with a deliberately shared credential (such as CommandCode's
            # OpenAI- and Anthropic-compatible profiles) resolve
            # deterministically to the first catalog entry; their distinct
            # base-URL settings still render independently.
            primary = d.api_key_env_vars[0]
            expected_by_primary.setdefault(primary, d)

        for primary, d in expected_by_primary.items():
...
class TestNewEndpoints:
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (coder_home / "config.yaml").touch()  # identity marker: bare dirs are not profiles
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        ...
        # Keep this row-shape test isolated from subscription discovery, which
        # legitimately resolves additional toolsets such as image_gen.
        monkeypatch.setattr(
            tools_config,
            "get_nous_subscription_features",
            lambda config, *, force_fresh=False: NousSubscriptionFeatures(
                subscribed=False,
        ...
        monkeypatch.setattr(
            tools_config,
            "_toolset_has_keys",
            lambda ts_key, config=None, features=None: ts_key != "web",
        )
        monkeypatch.setattr(
            toolsets_module,
            "resolve_toolset",
    ...
    def test_terminal_docker_probe_daemon_down(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "tools.environments.docker.find_docker",
            lambda: "/usr/bin/docker",
        )
        monkeypatch.setattr(
            "tools.environments.remote_common.run_capture",
    ...
    def test_terminal_probe_failure_is_a_status_not_a_500(self, monkeypatch):
        ...
        monkeypatch.setattr("tools.environments.remote_common.run_capture", boom)
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_keeps_relative_python_under_tui_cwd(
        ...
        # about argv/path resolution, not filesystem topology.
