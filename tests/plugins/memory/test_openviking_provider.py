...
def test_initialize_emits_starting_status_before_runtime_waiter_can_attach(monkeypatch):
    ...
    monkeypatch.setattr(
        openviking_module,
        "_start_local_openviking_server",
        lambda endpoint: (openviking_module._LOCAL_SERVER_STARTED, "started"),
    )
...
def test_runtime_openviking_waiter_warns_when_background_start_times_out(monkeypatch):
    ...
    assert warnings == [
        ...
        "within 60 seconds. OpenViking memory is temporarily unavailable; Hermes will retry "
        "on a later access or when the config changes."
    ]
...
def test_post_setup_local_server_down_can_offer_autostart(tmp_path, monkeypatch):
    ...
    def validate_reachability(endpoint):
        ...
        if len(reachability_calls) == 1:
            return False, "OpenViking server is not reachable."
        return True, ""
    ...
    monkeypatch.setattr(
        openviking_module,
        "_start_local_openviking_server",
        lambda endpoint: (started.append(endpoint) or openviking_module._LOCAL_SERVER_STARTED, "started"),
    )
    monkeypatch.setattr(openviking_module, "_wait_for_openviking_health", lambda endpoint, **kwargs: True)
    choices = iter([1, 0, 2, 0])
    monkeypatch.setattr(memory_setup, "_curses_select", lambda *args, **kwargs: next(choices))
    monkeypatch.setattr(
        memory_setup,
        "_prompt",
...
def test_on_memory_write_uses_content_write_independent_of_session_rotation():
    ...
    assert captured_payloads[0]["uri"].startswith(
        "viking://user/usr/peers/hermes/memories/preferences/mem_"
    )
...
def test_prefetch_can_return_session_start_memory_for_short_query():
    ...
    _mock_session_start_reads(
        provider,
        {
            ...
            ("/api/v1/fs/ls", "viking://user/default/memories/preferences"): [],
            ("/api/v1/fs/ls", "viking://user/default/memories/entities"): [],
        },
    )
...
def test_prefetch_session_start_reads_share_one_total_deadline(monkeypatch):
    ...
    def fake_get(path, params=None, **kwargs):
        ...
        if path == "/api/v1/system/status":
            return {"status": "ok", "result": {"user": "default"}}
        if (params or {}).get("uri") == "viking://user/default/memories/profile.md":
...
def test_prefetch_marks_successful_empty_session_start_memory_as_checked():
    ...
    assert first == ""
    assert second == ""
    assert [(path, params.get("uri", "")) for path, params, _timeout in calls] == [
        ("/api/v1/system/status", ""),
        ("/api/v1/content/read", "viking://user/default/memories/profile.md"),
        ("/api/v1/fs/ls", "viking://user/default/memories/preferences"),
        ("/api/v1/fs/ls", "viking://user/default/memories/entities"),
...
def test_prefetch_degrades_cleanly_when_profile_is_definitively_missing():
    ...
    assert "viking://user/default/memories/preferences/" in context
...
def test_session_start_memory_context_respects_total_budget_and_preserves_profile_tail(monkeypatch):
    ...
    _mock_session_start_reads(
        provider,
        {
            ("/api/v1/content/read", "viking://user/default/memories/profile.md"): long_profile,
            ("/api/v1/fs/ls", "viking://user/default/memories/preferences"): listing,
            ("/api/v1/fs/ls", "viking://user/default/memories/entities"): listing,
        },
    )
    provider._search_prefetch_context = MagicMock(return_value="should not run")
...
def test_prefetch_does_not_auto_inject_memory_overview_when_profile_missing():
    ...
    calls = _mock_session_start_reads(
        provider,
        {
            ("/api/v1/content/read", "viking://user/default/memories/profile.md"): RuntimeError(
