...
class TestCronDeliveryTargets:
    ...
    def test_unconfigured_platforms_excluded(self, monkeypatch):
        ...
        ids = {
            target["id"]
            for target in cron_delivery_targets()
...
class TestCronDeliveryMirror:
    ...
    def test_delivery_mirrors_opted_in_explicit_fanout_target(self):
        """An explicit target mirrors only with the job-level opt-in."""
        from gateway.config import Platform
    ...
    def test_delivery_mirrors_each_opted_in_explicit_fanout_target(self):
        """An explicit fan-out mirrors each declared target when opted in."""
        from gateway.config import Platform
...
class TestCronContinuableSurfaceInChannel:
    ...
    def test_in_channel_seeds_shared_channel_session_flat(self):
        ...
        # A companion thread-surface seed may be created for a reply in the
        # delivered brief's thread; this contract concerns the one flat session.
        flat_sources = [
            call.args[0]
        ...
        # Brief mirrored flat into that row. The companion reply-thread mirror
        # is covered separately; select the flat delivery by its thread id.
        flat_mirror_calls = [
            call
            for call in mirror_mock.call_args_list
            if call.kwargs.get("thread_id") is None
        ]
        assert len(flat_mirror_calls) == 1
        flat_mirror = flat_mirror_calls[0]
        assert flat_mirror.kwargs.get("thread_id") is None
    ...
    def test_in_channel_dm_seeds_dm_session(self):
        ...
        assert len(flat_mirror_calls) == 1
        assert flat_mirror_calls[0].kwargs.get("thread_id") is None
    ...
    def test_in_channel_from_origin_thread_delivers_flat_not_to_thread(self):
        ...
        # The flat session must still exist even when the delivery also seeds a
        # companion reply-thread surface for the delivered brief.
        flat_sources = [
            call.args[0]
            for call in adapter._session_store.get_or_create_session.call_args_list
            if call.args[0].thread_id is None
