...
def test_paused_creation_is_inert_until_operator_action(tmp_path, monkeypatch, make_cron_provider):
    ...
    with jobs.use_cron_store(tmp_path / "cron"):
        ...
        profiled = create(profile="default", paused=True, paused_reason="profile canary")
        assert profiled["success"], profiled
        assert profiled["job"]["profile"] == "default"
        assert profiled["job"]["state"] == "paused"
        assert profiled["job"]["paused_reason"] == "profile canary"
