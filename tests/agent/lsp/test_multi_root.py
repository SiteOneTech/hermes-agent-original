...
def test_multi_root_retires_an_idle_secondary_workspace(two_repos, mock_pyright, monkeypatch):
    repo_a, repo_b = two_repos
    _spawns, folders_log, install = mock_pyright
    install(multi_root=True)
    svc = _service()
    try:
        monkeypatch.chdir(str(repo_a))
        ...
        monkeypatch.chdir(str(repo_b))
        svc.get_diagnostics_sync(str(repo_b / "x.py"))
        # Keep the primary workspace active while aging only the secondary
        # worktree. A shared pyright process must release the stale watcher.
        monkeypatch.chdir(str(repo_a))
        svc.get_diagnostics_sync(str(repo_a / "x.py"))
        svc._root_last_used[("pyright", str(repo_b))] = time.time() - svc._idle_timeout - 1

        svc._loop.run(svc._reap_idle_once(), timeout=3)
