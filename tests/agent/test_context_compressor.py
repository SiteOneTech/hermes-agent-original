...
# HERMES_TEST_EXCLUSIVE: the isolation plugin spawns a process per test.
...
class TestSummaryTargetRatio:
    """Verify that summary_target_ratio scales the legacy-tail budget."""

    def test_tail_budget_scales_with_context(self):
        """Legacy tail budget should be threshold_tokens * summary_target_ratio."""
        with patch("agent.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(
                model="test", quiet_mode=True, summary_target_ratio=0.40, tail_mode="legacy",
            )
...
class TestTruncateToolCallArgsJson:
    ...
    def test_nested_structures_are_walked(self):
        ...
        assert parsed["messages"][0]["content"][200:].startswith(_COMPRESSION_MARKER_PREFIX)
        assert parsed["messages"][1]["content"] == "ok"
        assert parsed["meta"]["note"][200:].startswith(_COMPRESSION_MARKER_PREFIX)
    ...
    def test_scalar_json_string_gets_shrunk(self):
        ...
        assert parsed[200:].startswith(_COMPRESSION_MARKER_PREFIX)
...
class TestPreflightSentinelGuard:
    ...
    def test_real_value_not_revised_downward(self, compressor):
        ...
        compressor.maybe_seed_preflight_display_tokens(10_000)
