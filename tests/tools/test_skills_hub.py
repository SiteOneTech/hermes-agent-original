...
class TestGitHubSourceFileFetch:
    ...
    def test_fetch_uses_each_resolved_tree_blob_instead_of_tree_sha_as_ref(self, monkeypatch):
        src = GitHubSource(auth=MagicMock(spec=GitHubAuth))
        skill_sha = "a" * 40
        support_sha = "b" * 40
        tree_sha = "c" * 40
        ...
        payloads = {
            ...
            support_sha: b"guide bytes\n",
        }
        calls = []

        monkeypatch.setattr(src, "_get_repo_tree", lambda _repo: ("main", entries))
        src._tree_revisions["owner/repo"] = tree_sha

        def fetch_bytes(repo, path, *, blob_sha=None, ref=None):
            calls.append((repo, path, blob_sha, ref))
            assert isinstance(blob_sha, str)
            return payloads.get(blob_sha)

        monkeypatch.setattr(src, "_fetch_file_bytes", fetch_bytes)

        bundle = src.fetch("owner/repo/skills/demo")

        assert bundle is not None
        assert bundle.files == {
            "SKILL.md": payloads[skill_sha].decode("utf-8"),
            "references/guide.md": payloads[support_sha],
        }
        assert [(path, blob_sha, ref) for _repo, path, blob_sha, ref in calls] == [
            ("skills/demo/SKILL.md", skill_sha, None),
            ("skills/demo/references/guide.md", support_sha, None),
        ]
        assert bundle.metadata["source_revision"] == tree_sha
...
class TestRepoRootSkillLayout:
    ...
    ROOT_TREE = [
        {"path": "LICENSE", "type": "blob", "sha": "a" * 40},
        {"path": "README.md", "type": "blob", "sha": "b" * 40},
        {"path": "SKILL.md", "type": "blob", "sha": "c" * 40},
        {"path": "references/free_tools.md", "type": "blob", "sha": "d" * 40},
    ]
    ...
    def _github_stub(self, tree):
        ...
        def _side_effect(url, **kwargs):
            ...
            if url.rstrip("/").endswith(self.REPO):
                ...
                contents = {
                    "a" * 40: b"root-level file\n",
                    "b" * 40: b"root-level file\n",
                    "c" * 40: self.SKILL_MD.encode(),
                    "d" * 40: b"# Free tools\n",
                }
                resp.status_code, resp.content = 200, contents[url.rsplit("/", 1)[-1]]
