...
class TestUnicodeVariantRepair:
    ...
    def test_newline_named_sibling_does_not_corrupt_target_listing(self, ws):
        # The candidate enumerator uses target-side Python + base64 JSON, not
        # newline-delimited ``ls`` output. A legal newline in another filename
        # must not corrupt the equivalent-name match.
        (ws / "notes" / "unrelated\nentry.txt").write_text("ignore\n")
        result = json.loads(read_file_tool(str(ws / "notes" / CLEAN)))
        assert "rotate the keys" in result.get("content", "")
        assert "unicode-equivalent" in result.get("hint", "")
