...
def test_github_fetches_each_tree_entry_as_an_immutable_blob(monkeypatch):
    """Every byte comes from the validated tree entry, never a tree SHA ref."""
    source = GitHubSource(GitHubAuth())
    fetched: list[tuple[str, str | None, str | None]] = []
    skill_sha = "a" * 40
    sibling_sha = "b" * 40

    def _fake_content(_repo, path, ref=None, *, blob_sha=None):
        fetched.append((path, blob_sha, ref))
    ...
    monkeypatch.setattr(
        source,
        "_fetch_file_bytes",
        lambda _repo, path, *, blob_sha=None, ref=None: fetched.append((path, blob_sha, ref)) or b"x",
    )
    source._tree_cache["owner/repo"] = (
        "main",
        [
            {"path": "skill/SKILL.md", "type": "blob", "mode": "100644", "sha": skill_sha},
            {"path": "skill/CONTEXT-FORMAT.md", "type": "blob", "mode": "100644", "sha": sibling_sha},
        ],
    )
    ...
    assert bundle is not None
    assert fetched == [
        ("skill/SKILL.md", skill_sha, None),
        ("skill/CONTEXT-FORMAT.md", sibling_sha, None),
    ]


def test_github_source_rejects_symlink_in_referenced_directory(monkeypatch):
    source = GitHubSource(GitHubAuth())
    monkeypatch.setattr(source, "_fetch_file_content", lambda _repo, path, ref=None, *, blob_sha=None: SKILL_MD if path.endswith("SKILL.md") else "x")
    source._tree_cache["owner/repo"] = (
        "main",
        [
            {"path": "skill/SKILL.md", "type": "blob", "mode": "100644", "sha": "a" * 40},
...
def test_github_source_fetch_downloads_full_skill_directory(monkeypatch):
    ...
    fetched: list = []
    monkeypatch.setattr(source, "_fetch_file_content", lambda _repo, path, ref=None, *, blob_sha=None: skill_md)
    monkeypatch.setattr(
        source, "_fetch_file_bytes",
        lambda _repo, path, *, blob_sha=None, ref=None: fetched.append(path) or b"content-of-" + path.encode(),
    )
    source._tree_cache["owner/repo"] = (
        "main",
        [
...
def test_github_source_fetch_dangling_linked_reference_warns_not_aborts(monkeypatch):
    ...
    monkeypatch.setattr(source, "_fetch_file_content", lambda _repo, path, ref=None, *, blob_sha=None: skill_md)
    monkeypatch.setattr(source, "_fetch_file_bytes", lambda _repo, path, *, blob_sha=None, ref=None: b"x")
