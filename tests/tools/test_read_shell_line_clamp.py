"""Descriptor-safe per-line clamp in ShellFileOperations.read_file.

A file with one pathological line (e.g. a 400MB minified bundle on a single
line) used to cross the exec transport in full because Python's file iterator
materialized that line before the display clamp ran. The target-side
descriptor-safe reader now walks bounded chunks and retains only a bounded
prefix for selected lines before the bytes cross the transport.

Byte-vs-char note: the reader retains up to four bytes per display character,
then incrementally decodes a truncated prefix. This preserves valid UTF-8 at a
byte boundary, lets the caller's character clamp add ``... [truncated]``, and
does not launder genuinely invalid binary bytes into text.
"""
