...
class TestPastEofNote:
    ...
    def test_offset_at_unterminated_last_line_still_reads(self, tmp_path, monkeypatch):
        """The final line counts even when the file has no terminal newline."""
        monkeypatch.setenv("TERMINAL_CWD", str(tmp_path))
        p = tmp_path / "unterminated.txt"
        p.write_text("a\nb")
        result = json.loads(read_file_tool(str(p), offset=2, limit=10))
        assert "2|b" in result.get("content", "")
        assert "beyond" not in (result.get("hint") or "")

    def test_offset_after_unterminated_last_line_names_recovery(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TERMINAL_CWD", str(tmp_path))
        p = tmp_path / "unterminated.txt"
        p.write_text("a\nb")
        result = json.loads(read_file_tool(str(p), offset=3, limit=10))
        assert "beyond the end" in (result.get("hint") or "")
        assert "2" in (result.get("hint") or "")
