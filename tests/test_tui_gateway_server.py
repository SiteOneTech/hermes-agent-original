...
def test_write_json_serializes_concurrent_writes(monkeypatch):
    ...
    seq_frames = [
        frame
        for frame in frames
        if frame.get("text") == "x" * 24 and "seq" in frame
    ]

    # Other asynchronous gateway work can emit a valid frame while this test
    # owns the process-wide stdout mock. Every line must still be intact JSON,
    # and each of this test's concurrent writes must appear exactly once.
    assert len(seq_frames) == 8
    assert {frame["seq"] for frame in seq_frames} == set(range(8))
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # `kanban` and `minimax_cli` are auto-recovered non-configurable toolsets;
    # `project` is GUI-only. Newly released toolsets are back-filled too, so
    # tolerate only the explicit transition set beyond this stable baseline.
    from hermes_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS

    result = server._load_enabled_toolsets()
    assert result is not None
    expected = {"kanban", "memory", "minimax_cli", "project"}
