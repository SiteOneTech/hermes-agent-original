...
# Runtime repair atomically parks a replaced virtualenv beside the live one as
# ``venv.stale.runtime-<token>``. These backup contents are third-party code,
# not source files the managed-runtime guard owns.
_EXEMPT_DIR_PREFIXES = ("venv.", ".venv.")
...
def _source_files() -> list[Path]:
    ...
    for dirpath, dirnames, filenames in os.walk(REPO_ROOT):
        ...
        dirnames[:] = [
            d
            for d in dirnames
            if d not in _EXEMPT_DIRS
            and not d.startswith(_EXEMPT_DIR_PREFIXES)
            and (rel_dir != Path(".") or not _is_packaging_copy(d))
        ]
...
def test_source_files_skip_nested_stale_virtualenv_backups(tmp_path, monkeypatch):
    """Runtime-repair backups are virtualenv contents, wherever they reside."""
    (tmp_path / "app.py").write_text("pass\n", encoding="utf-8")
    # A source file with this name must remain visible: only directory names
    # are exempted from the repository-wide scan.
    (tmp_path / "venv.py").write_text("pass\n", encoding="utf-8")
    stale_file = (
        tmp_path
        / "workspace"
        / "venv.stale.runtime-example"
        / "lib"
        / "third_party.py"
    )
    stale_file.parent.mkdir(parents=True)
    stale_file.write_text("from shutil import which\nwhich('uv')\n", encoding="utf-8")
    monkeypatch.setattr(sys.modules[__name__], "REPO_ROOT", tmp_path)

    assert {path.relative_to(tmp_path) for path in _source_files()} == {
        Path("app.py"),
        Path("venv.py"),
    }
