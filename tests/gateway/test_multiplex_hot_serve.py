...
@pytest.mark.asyncio
async def test_transient_atomic_secret_scope_failure_retries_through_real_start_path(tmp_path, monkeypatch):
    """A secondary startup must carry one atomic off-loop secret scope into its adapters."""
    ...
    hydrate_calls = []
    real_hydrate = env_loader.hydrate_and_build_profile_secret_scope
    ...
    monkeypatch.setattr(env_loader, "hydrate_and_build_profile_secret_scope", _flaky)
    ...
    assert hydrate_calls == [str(gamma_dir), str(gamma_dir)]
