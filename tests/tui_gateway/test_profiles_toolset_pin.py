...
def test_composite_pin_keeps_native_member_toolsets():
    """A composite remains an opt-in to its dynamically registered native members."""
    cfg: dict = {}
    resolved, pinned = _pin_and_resolve(cfg, ["hermes-cli"])
    assert pinned == {"hermes-cli"}
    assert "minimax_cli" in resolved
