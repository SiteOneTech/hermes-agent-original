...
class GatewayAdapterLifecycleMixin:
    ...
    async def _load_secondary_profile_config(self, profile_name: str, profile_home: "Path"):
        ...
        # Build the immutable scope off-loop. Passing it into every nested
        # scope below closes the reset-between-hydrate-and-build race without
        # blocking gateway heartbeats on an external secret source.
        profile_secret_scope = await asyncio.to_thread(
            hydrate_and_build_profile_secret_scope, profile_home
        )
        with _profile_runtime_scope(profile_home, prepared_secret_scope=profile_secret_scope):
        ...
        return profile_cfg, profile_secret_scope
    ...
    async def _start_one_profile_adapters(
        ...
        profile_cfg, profile_secret_scope = await self._load_secondary_profile_config(
    ...
    async def _secondary_reconnect_attempt(self, profile_name: str, platform: Platform):
        ...
        # Hydrate and materialize one immutable scope off-loop so a concurrent
        # cache reset cannot clear the snapshot before adapter construction.
        profile_secret_scope = await asyncio.to_thread(
            hydrate_and_build_profile_secret_scope, profile_home
        )
        with _profile_runtime_scope(profile_home, prepared_secret_scope=profile_secret_scope):
        ...
        try:
            while self._running:
                ...
                with self._scope_or_null(
                        functools.partial(_profile_runtime_scope, prepared_secret_scope={}), profile_home):
