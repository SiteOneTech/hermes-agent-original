...
def _warm_turn_machinery_sync() -> int:
    ...
    TTL cache), the local Python toolchain and optional STT backend probes (#106064), and the default route's
    context-window metadata (#105986) — catalog and model-load work that must not sit between the first inbound
    turn and its inference request. Context files remain lazy because they need the active turn's agent."""
    ...
    try:
        # STT is lazy too (local Whisper loads its weights on first use), so a cold first voice
        # note would otherwise pay the model load mid-turn.
...
def _load_profile_secret_scope(profile_home: "Path") -> dict:
    ...
    try:
        return hydrate_and_build_profile_secret_scope(Path(profile_home))
    finally:
