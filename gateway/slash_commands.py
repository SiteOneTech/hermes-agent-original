...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_retry_command(self, event: MessageEvent) -> str:
        ...
        if handoff is not None:
            ...
            # as active=0/compacted=1 rows under this same session id, and a
            # bare rewrite (active_only=False) would DELETE them (same class as
            # #61145). /retry never intends to purge archived history, and
            # active_only preserves that archive; a separate existence probe
            # could fail open or race with the write.
