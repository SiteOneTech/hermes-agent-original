...
class GatewayInboundMixin:
    ...
    # Adapter stand-ins for "this message was media only" (matched case-insensitively). The media note
    # we prepend already says that, so keeping them makes the agent answer the marker instead.
    _REDUNDANT_MEDIA_TEXTS = frozenset({_EMPTY_TEXT_PLACEHOLDER.lower(), "[audio received]"})

    @classmethod
    def _prepend_media_prefix(cls, prefix: str, user_text: str) -> str:
        """``prefix`` + the user's text; media-only placeholders (Discord's empty-content marker,
        ``[audio received]``) are dropped as redundant."""
        if user_text and user_text.strip().lower() not in cls._REDUNDANT_MEDIA_TEXTS:
            return f"{prefix}\n\n{user_text}"
        return prefix
