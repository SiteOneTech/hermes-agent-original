...
- Canonical Factory status: `agent_core_postgres:zeus_agent.factory` still reports `unvalidated_required_docs`; the R2n dispatch snapshot reported 10 blocking G1 rows plus 4 heuristic false-ready rows (`SPRINT_PLAN.md`, `TRACKER.md`, `DOCUMENTATION_INDEX.md`, `QA_GATES.md`), while strict primary read-back from project `repo_path=/home/jean/Projects/hermes-agent-original` reads all 14 required G1 documents as `exists=true`, `indexed=true`, `committed=true`, `validated=true`, `reviewed=false`, `readiness_source=primary`.
...
- R2m superseded R2k after current-base recovery, but R2n now supersedes R2m as the active review target: the next valid review target is the exact SHA on the R2n Zeus-signed `agent:zeus` PR based on `origin/main` `df4c77fd1413a65cdb85885a06978ff157c1de4d`; R2k/PR #31 and R2m are historical evidence only. The R2n correction also removes branch-local positive review-marker prose from the first 40 lines of the four false-ready control docs so the intended pending state matches their frontmatter and index entries.
