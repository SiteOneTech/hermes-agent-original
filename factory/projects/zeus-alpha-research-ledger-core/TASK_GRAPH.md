...
### R2v/R2w/R2y configured-base document-status reconciliation

- R2v repaired the control-plane contract so stale primary-checkout blockers are superseded only by verified configured-base-ref content and never by arbitrary candidate PR/worktree metadata; it also blocks Factory auto-integration for projects with `factory_auto_integration_forbidden=true`.
- R2w recorded the first PR-first reviewed-frontmatter status read-back at configured base `origin/main` `df79aac9d306c0b055fe88dbde5ebd54d9635e36` with zero required G1 blockers and independent quality review evidence for exact SHA `ce79f0159694fda5e74de7cd5913ab0e2704e2a7`.
- R2y records the current source-alignment check: assigned branch/worktree `factory/zeus-alpha-research-ledger-core/inc-001-r2y-repair-canonical-g1-document` starts from `origin/main` `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7`; approved Factory status output `out-1786870710-863987-8590.log` shows all 14 required G1 rows with `readiness_source=configured_base_ref`, `configured_base_ref_accepted=true`, `reviewed=true`, and `blocking=false`.
- Reconciler events or project metadata that still name `unvalidated_required_docs` are stale drift unless they identify a current blocking `project.document_status` row. They do not authorize implementation dispatch; they only inform the PR-first R2y review handoff.
...
| `zeus-alpha-research-ledger-core-r2v-canonical-g1-status-and-no-auto-merg` | documentation / done | codex-builder → quality-reviewer | stale primary status + no-auto-merge contract | `factory/zeus-alpha-research-ledger-core/inc-005-r2v-canonical-g1-status-and-no-a` | `/home/jean/Projects/.worktrees/zeus-alpha-research-ledger-core/inc-005-r2v-canonical-g1-status-and-no-a` |
| `zeus-alpha-research-ledger-core-r2w-canonical-g1-reviewed-frontmatter-pr` | documentation / done | codex-builder → quality-reviewer | configured-base reviewed-frontmatter PR recovery | `factory/zeus-alpha-research-ledger-core/inc-001-r2w-canonical-g1-reviewed-frontm` | `/home/jean/Projects/.worktrees/zeus-alpha-research-ledger-core/inc-001-r2w-canonical-g1-reviewed-frontm` |
| `zeus-alpha-research-ledger-core-r2y-repair-canonical-g1-document-status-` | documentation / claimed | codex-builder → quality-reviewer | source-alignment repair for stale `unvalidated_required_docs` reports after zero-blocker status read-back | `factory/zeus-alpha-research-ledger-core/inc-001-r2y-repair-canonical-g1-document` | `/home/jean/Projects/.worktrees/zeus-alpha-research-ledger-core/inc-001-r2y-repair-canonical-g1-document` |
...
- R2y confirms the current row-level `project.document_status` evaluation at configured base `origin/main` `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7` still has zero required G1 blockers; stale reconciler event metadata alone cannot re-block G1 readiness without a current blocking row.
