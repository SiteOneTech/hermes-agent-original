...
- Factory CLI status: `/home/jean/Projects/hermes-agent-original/venv/bin/python3 -m hermes_cli.main factory status --json zeus-alpha-research-ledger-core` from the assigned worktree; exit `0`; final post-commit verification output cached at `/home/jean/.hermes/profiles/codex-builder/cache/terminal-output/out-1786924687-620586-6c50.log`.
...
- Evidence lines: `/home/jean/.hermes/profiles/codex-builder/cache/terminal-output/out-1786924687-620586-6c50.log` lines `19272`–`19622`.
...
|---|---|---|---|
| `FACTORY_INTAKE.md` | log lines 19273–19297 | reviewed/validated and non-blocking | stale critical-readiness gate snapshots, e.g. gate 845 lines 8035–8067 |
| `REQUIREMENTS_ANALYSIS.md` | log lines 19298–19322 | reviewed/validated and non-blocking | stale gate 845 lines 8068–8081 |
| `PATTERN_ANALYSIS.md` | log lines 19323–19347 | reviewed/validated and non-blocking | stale gate 845 lines 8082–8095 |
| `ASSUMPTIONS_AND_OPEN_QUESTIONS.md` | log lines 19348–19372 | reviewed/validated and non-blocking | stale gate 845 lines 8096–8109 |
| `PRD.md` | log lines 19373–19397 | reviewed/validated and non-blocking | stale gate 845 lines 8110–8123 |
| `ADRS.md` | log lines 19398–19422 | reviewed/validated and non-blocking | stale gate 845 lines 8124–8137 |
| `METHODOLOGY_PLAN.md` | log lines 19423–19447 | reviewed/validated and non-blocking | stale gate 845 lines 8138–8151 |
| `TECHNICAL_BLUEPRINT.md` | log lines 19448–19472 | reviewed/validated and non-blocking | stale gate 845 lines 8152–8165 |
| `SPRINT_PLAN.md` | log lines 19473–19497 | reviewed/validated and non-blocking | stale gate 845 lines 8166–8179; later human summary omitted it from the blocker list |
| `TASK_GRAPH.md` | log lines 19498–19522 | reviewed/validated and non-blocking | stale gate 845 lines 8180–8193 |
| `TRACKER.md` | log lines 19523–19547 | reviewed/validated and non-blocking | not a current blocker in either current-origin readback or the latest human summary |
| `DOCUMENTATION_INDEX.md` | log lines 19548–19572 | reviewed/validated and non-blocking | not a current blocker in either current-origin readback or the latest human summary |
| `QA_GATES.md` | log lines 19573–19597 | reviewed/validated and non-blocking | not a current blocker in either current-origin readback or the latest human summary |
| `SECURITY_GATES.md` | log lines 19598–19622 | reviewed/validated and non-blocking | stale gate 845 lines 8236–8249 |

## Remaining provenance/status mismatch

The remaining anomaly is not the current-origin document rows. It is stale persisted/reconciler provenance:

- Recent Factory events still record `anomalies=["unvalidated_required_docs"]` before this worker claim, for example event `192994` at log lines `470`–`493`.
- Project metadata still carries `reconciliation_anomalies=["unvalidated_required_docs"]` and `reconciliation_required=true` at log lines `19870`–`19873`, while the same status payload's live `document_status` rows are non-blocking.
- Project metadata still names the obsolete `g1_documentation_checkout` PR #20 / commit `dad375f27568c38be771fc597b579d087f034e1d` at log lines `19837`–`19845`; current row-level status instead reads the configured base `origin/main` at `139df9ae49137bb4b16152550d53d385310de3b6`.
...
Bounded follow-up repair, if a later dispatcher still blocks after this current-origin PR/review:
