...
- `R2Y_CANONICAL_G1_DOCUMENT_STATUS_SOURCE_ALIGNMENT.md` records the current source-alignment repair: assigned branch/worktree at `origin/main` `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7`, Factory status CLI read-back from the repository venv with every required G1 row `readiness_source=configured_base_ref`, `reviewed=true`, and `blocking=false`, plus the PR-first exact-SHA review handoff for R2y.
...
- R2y status evidence confirms the live `project.document_status` rows still read from the configured base ref, now `origin/main` commit `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7`, and report zero required G1 blockers even while recent reconciler event metadata may still contain stale `unvalidated_required_docs` history. Reviewers must use the row-level `document_status` source/commit tuple for G1 readiness decisions.
...
4. `QA_GATES.md`, `SECURITY_GATES.md`, `G1_REVIEW.md`, `R2K_STALE_CANONICAL_G1_PROVENANCE_REPAIR.md`, `R2M_CURRENT_BASE_G1_REVIEW_HANDOFF.md`, `R2U_CANONICAL_G1_DOCUMENT_STATUS_PREFLIGHT_REPAIR.md`, `R2V_CANONICAL_G1_STATUS_AND_NO_AUTO_MERGE_REPAIR.md`, `R2W_CANONICAL_G1_REVIEWED_FRONTMATTER_PR.md`, `R2Y_CANONICAL_G1_DOCUMENT_STATUS_SOURCE_ALIGNMENT.md`
