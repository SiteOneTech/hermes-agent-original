...
- `R2AG_CURRENT_BASE_G1_DOCUMENT_STATUS_REPAIR.md` records the current-base technical reconciliation evidence for R2ag: exact `origin/main` `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7`, project-level Factory `document_status` read-back with zero G1 required-document blockers, and the remaining stale `metadata.reconciliation_anomalies=["unvalidated_required_docs"]` drift if the reconciler does not clear it after this PR/gate evidence.
...
- R2ag current-base evidence confirms the same configured-base status path on current `origin/main` commit `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7` reports all 14 G1 required rows with `exists/committed/indexed/validated/reviewed=true` and `blocking=false`; any remaining `unvalidated_required_docs` flag is reconciliation-metadata drift, not a current document-content blocker.
...
4. `QA_GATES.md`, `SECURITY_GATES.md`, `G1_REVIEW.md`, `R2K_STALE_CANONICAL_G1_PROVENANCE_REPAIR.md`, `R2M_CURRENT_BASE_G1_REVIEW_HANDOFF.md`, `R2U_CANONICAL_G1_DOCUMENT_STATUS_PREFLIGHT_REPAIR.md`, `R2V_CANONICAL_G1_STATUS_AND_NO_AUTO_MERGE_REPAIR.md`, `R2AG_CURRENT_BASE_G1_DOCUMENT_STATUS_REPAIR.md`
