...
R2r is the PR-first recovery of that R2q candidate. It uses source commit `11639ab1650a4d7abfa88820bc266c983a56d1fd` on parent/base `df4c77fd1413a65cdb85885a06978ff157c1de4d`, because the R2q source branch had no open GitHub PR and its source commit was authored by `sitiouno` without a Zeus sign-off. R2r restores the same docs-only candidate under the assigned replacement branch/worktree and adds only the R2r provenance artifact; the replacement head SHA must be bound in the PR body and independent review because embedding it here would change the commit.
...
This is **candidate readiness**, not primary-repo readiness. No normal ALR-020+ dispatch is authorized until canonical Agent Core `document_status` or an explicitly authorized reviewed-candidate path reads back no required G1 blockers and an independent `solution-architect` review cites the exact final R2r replacement PR head SHA and proves review work actually ran.
...
- `R2R_PR_FIRST_RECOVERY_OF_R2Q_G1_REVIEWED_DOCS_CANDIDATE.md` records the source R2q SHA, replacement branch/worktree/base identity, Zeus-signoff requirement, open-PR requirement and exact-head independent solution-architect review contract for this R2r replacement.
...
- R2q/R2r remains pending independent `solution-architect` review until the reviewer cites the final R2r replacement PR head SHA and proves review execution with real tool/file/diff/command evidence.
...
4. `QA_GATES.md`, `SECURITY_GATES.md`, `G1_REVIEW.md`, `R2K_STALE_CANONICAL_G1_PROVENANCE_REPAIR.md`, `R2M_CURRENT_BASE_G1_REVIEW_HANDOFF.md`, `R2Q_CURRENT_MAIN_G1_REVIEW_CANDIDATE_RECOVERY.md`, `R2R_PR_FIRST_RECOVERY_OF_R2Q_G1_REVIEWED_DOCS_CANDIDATE.md`

## G1 rule
No normal implementation starts from the R2q source branch or the R2r replacement branch alone. It may start only after the reviewed candidate is accepted into the canonical Factory source path or authorized reviewed-candidate metadata, Agent Core reads back no required G1 blockers, and the independent solution-architect review of the exact final R2r replacement PR head SHA is valid. This protects against false-ready assertions while preserving the independently reviewed candidate markers.
