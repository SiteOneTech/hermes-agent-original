...
- `R2N_CANONICAL_DOCUMENT_STATUS_REPAIR.md` supersedes R2m as the active handoff for this repair. It reproduces the live Agent Core `document_status` read-back on primary repo path `/home/jean/Projects/hermes-agent-original`, records all 14 required G1 documents as `exists=true`, `indexed=true`, `committed=true`, `validated=true`, `reviewed=false`, and binds the next valid review to the fresh R2n PR head SHA after push.
