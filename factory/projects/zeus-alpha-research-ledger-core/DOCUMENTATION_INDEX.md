...
- `R2AF_DISPATCHABLE_CANONICAL_REQUIRED_DOC_VALIDATION.md` records the current dispatchable required-document validation repair: stale blocker snapshots had only `reviewed=false`, while the current approved Factory CLI `document_status` read-back sees all 14 required G1 documents reviewed/validated/indexed/committed and non-blocking on configured base `origin/main` `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7`.
...
- R2w/R2af status evidence confirms the configured-base `document_status` reader now sees the reviewed frontmatter/index state on `origin/main` through current commit `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7` with all G1 required rows non-blocking; this confirmation remains review evidence, not a downstream implementation authorization.
...
4. `QA_GATES.md`, `SECURITY_GATES.md`, `G1_REVIEW.md`, `R2K_STALE_CANONICAL_G1_PROVENANCE_REPAIR.md`, `R2M_CURRENT_BASE_G1_REVIEW_HANDOFF.md`, `R2U_CANONICAL_G1_DOCUMENT_STATUS_PREFLIGHT_REPAIR.md`, `R2V_CANONICAL_G1_STATUS_AND_NO_AUTO_MERGE_REPAIR.md`, `R2AF_DISPATCHABLE_CANONICAL_REQUIRED_DOC_VALIDATION.md`
