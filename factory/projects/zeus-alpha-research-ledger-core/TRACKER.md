...
| R2n canonical-document repair | active bounded handoff | Assigned branch/worktree recreated from current `origin/main` `df4c77fd1413a65cdb85885a06978ff157c1de4d`; canonical Factory status still reads all 14 required G1 docs from primary repo path as `reviewed=false`; next valid review target is the fresh R2n PR head SHA. See `R2N_CANONICAL_DOCUMENT_STATUS_REPAIR.md`. |
...
- R2n supersedes R2m as active handoff on current `origin/main` `df4c77fd1413a65cdb85885a06978ff157c1de4d`, records the full 14-document canonical `reviewed=false` read-back, and keeps `reviewed: pending` until independent reviewers inspect the R2n PR head.
