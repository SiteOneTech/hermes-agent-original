...
| R2m current-base recovery | historical | R2m recorded base `ab08b13669903a87b3d60d6c80231d23d6313782`; it is superseded by R2n/R2o and remains provenance only. |
...
| R2o reviewed markers | candidate reviewed, primary hold | Required G1 docs now carry `validated: yes` and `reviewed: yes` with gate 789/PR #33 exact SHA metadata. This is candidate readiness only; primary readiness still requires canonical Factory read-back after PR/source reconciliation. |
...
- R2m now records the current canonical base `ab08b13669903a87b3d60d6c80231d23d6313782`, adds a fresh exact-SHA handoff artifact, and is historical after R2n/R2o.
...
- R2o applies explicit candidate-level `reviewed: yes` markers to the 14 required G1 docs, backed by gate 789/`quality-reviewer`/PR #33 SHA; it does not claim primary readiness before canonical source reconciliation.
...
Commit and push the bounded R2o marker-application branch, open/update a Zeus-signed `agent:zeus` PR/handoff that names the final branch head SHA, run the targeted document-status tests, and record independent quality review evidence against that exact R2o SHA. Product implementation, merge to primary, deploy, runtime work and ALR-020+ dispatch remain blocked until canonical Agent Core document readiness or authorized reviewed-candidate metadata reads back zero required G1 blockers.
