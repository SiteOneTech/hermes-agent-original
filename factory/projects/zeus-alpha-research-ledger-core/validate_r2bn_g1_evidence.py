#!/usr/bin/env python3
"""Validate R2bn canonical G1 review-state source-root evidence.

This validator is intentionally deterministic and read-only. It validates a
Factory status JSON captured by the approved canonical CLI plus the committed
project-local Markdown pack in the assigned worktree. It fails if the status is
from the wrong source root, if required G1 rows are not clean from the current
configured base, if stale required-doc anomalies appear as active metadata, or
if PR/quality-gate evidence is missing for the exact final candidate head.
"""
...
ARTIFACT = "R2BN_CANONICAL_G1_REVIEW_STATE_SOURCE_ROOT_REPAIR.md"
EXPECTED_SOURCE_ROOT = "/home/jean/Projects/.worktrees/zeus-alpha-research-ledger-core/inc-018-r2bn-canonical-g1-review-state-s"
...
def validate(args: argparse.Namespace) -> list[str]:
    ...
    _require(data.get("factory_cli_source_root") == EXPECTED_SOURCE_ROOT, "status factory_cli_source_root is not assigned R2bn worktree", failures)
    _require(data.get("factory_status_source_root") == EXPECTED_SOURCE_ROOT, "status factory_status_source_root is not assigned R2bn worktree", failures)
    ...
    for task in data.get("tasks") or []:
        ...
        if task_id == TASK_ID:
            _require("unvalidated_required_docs" not in json.dumps(task_metadata, sort_keys=True), "R2bn task itself carries unvalidated_required_docs metadata", failures)
            continue
    ...
    # post-push review gate exists. The exact final head/gate binding is
    # validated from canonical Factory status below so we do not create an
    # impossible self-referential commit that must contain its own SHA/gate id.
...
def main() -> int:
    ...
    if failures:
        print("R2BN_G1_EVIDENCE_VALIDATION=FAIL")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("R2BN_G1_EVIDENCE_VALIDATION=PASS")
