#!/usr/bin/env python3
"""Validate R2ct canonical G1 documentation evidence without touching Factory DB.

The validator is intentionally read-only. It validates a Factory status JSON
captured with the approved CLI from the assigned R2ct worktree plus the
committed project-local Markdown pack. It fails if G1 rows are not clean from
current configured origin/main, if the 14 required document frontmatter markers
lose their independent PR #36/gate 794 provenance, if R2bn PR #80/gate 929
handoff is not recorded as historical current-base evidence, or if the R2ct PR
head and independent quality gate are not bound to the exact final candidate.
"""
...
ARTIFACT = "R2CT_BOUNDED_CANONICAL_G1_DOCUMENTATION_VALIDATION_PR_FIRST_RECOVERY.md"
EXPECTED_SOURCE_ROOT = "/home/jean/Projects/.worktrees/zeus-alpha-research-ledger-core/inc-019-r2ct-bounded-canonical-g1-docume"
...
CANDIDATE_MARKERS = [
    "candidate readiness",
    "primary origin/main readiness",
    "current configured-base rows",
]
...
def _validate_residual_tasks(data: dict[str, Any], failures: list[str]) -> None:
    ...
    for task in data.get("tasks") or []:
        ...
        if task_id in {TASK_ID, PREDECESSOR_TASK_ID}:
            _require(
...
def _validate_repair_docs(project_dir: Path, args: argparse.Namespace, failures: list[str]) -> None:
    expected_markers = [
        args.expected_base,
        args.expected_pr,
        args.predecessor_base,
        args.predecessor_head,
        args.predecessor_pr,
        str(args.predecessor_quality_gate),
    ]
    ...
    _require("structured_reconciliation_metadata" in normalized, f"{ARTIFACT} does not name structured residual source", failures)
    _require("r2ai" in normalized and "r2ae" in normalized, f"{ARTIFACT} does not name residual stale tasks", failures)
    _require("14/14" in normalized, f"{ARTIFACT} does not record 14/14 G1 rows", failures)


def _validate_predecessor_gate(data: dict[str, Any], args: argparse.Namespace, failures: list[str]) -> None:
    predecessor_gate = _gate(data, args.predecessor_quality_gate)
    _require(predecessor_gate is not None, f"predecessor quality gate {args.predecessor_quality_gate} absent from status JSON", failures)
    if not predecessor_gate:
        return
    notes = str(predecessor_gate.get("notes") or "")
    _require(predecessor_gate.get("project_id") == PROJECT_ID, f"gate {predecessor_gate.get('gate_id')} project mismatch", failures)
    _require(predecessor_gate.get("gate_type") == "quality", f"gate {predecessor_gate.get('gate_id')} type mismatch", failures)
    _require(predecessor_gate.get("status") == "passed", f"gate {predecessor_gate.get('gate_id')} did not pass", failures)
    _require(predecessor_gate.get("reviewer") == "quality-reviewer", f"gate {predecessor_gate.get('gate_id')} reviewer mismatch", failures)
    for marker in (PREDECESSOR_TASK_ID, args.predecessor_base, args.predecessor_head, args.predecessor_pr):
        _require(marker in notes, f"predecessor gate notes missing {marker}", failures)
    _require("no direct SQL" in notes and "no merge" in notes, "predecessor gate notes missing safety boundary", failures)


def _validate_r2ct_quality_gate(data: dict[str, Any], args: argparse.Namespace, failures: list[str]) -> None:
    ...
    # Task-scoped passed gates may auto-integrate in the current CLI for this project.
    # R2ct is explicitly no-merge, so a project-scoped gate is accepted only when
    # immutable notes bind the task id below.
...
def validate(args: argparse.Namespace) -> list[str]:
    ...
    _require(data.get("factory_cli_source_root") == EXPECTED_SOURCE_ROOT, "status factory_cli_source_root is not assigned R2ct worktree", failures)
    _require(data.get("factory_status_source_root") == EXPECTED_SOURCE_ROOT, "status factory_status_source_root is not assigned R2ct worktree", failures)
    ...
    _validate_predecessor_gate(data, args, failures)
    _validate_r2ct_quality_gate(data, args, failures)

    for attr in ("expected_base", "expected_head", "predecessor_base", "predecessor_head"):
        value = getattr(args, attr)
        if not re.fullmatch(r"[0-9a-f]{40}", value):
            failures.append(f"{attr} is not a 40-character SHA")
    if args.expected_head == args.expected_base:
        failures.append("expected head equals base; no R2ct commit was validated")
    if args.expected_base == args.predecessor_head:
        failures.append("expected base equals predecessor head; candidate/main readiness is not distinguished")
    for attr in ("expected_pr", "predecessor_pr"):
        value = getattr(args, attr)
        if not re.fullmatch(r"https://github\.com/SiteOneTech/hermes-agent-original/pull/[0-9]+", value):
            failures.append(f"{attr} is not a SiteOneTech PR URL")
...
def main() -> int:
    ...
    parser.add_argument("--expected-base", required=True, help="Exact current origin/main base SHA for R2ct")
    parser.add_argument("--expected-head", required=True, help="Exact final pushed R2ct PR head SHA")
    parser.add_argument("--expected-pr", required=True, help="Exact non-draft R2ct GitHub PR URL")
    parser.add_argument("--expected-quality-gate", required=True, type=int, help="Canonical Factory quality gate id for R2ct")
    parser.add_argument("--predecessor-base", required=True, help="Exact R2bn base SHA")
    parser.add_argument("--predecessor-head", required=True, help="Exact R2bn PR #80 head SHA")
    parser.add_argument("--predecessor-pr", required=True, help="Exact R2bn PR URL")
    parser.add_argument("--predecessor-quality-gate", required=True, type=int, help="Canonical Factory quality gate id for R2bn")
    args = parser.parse_args()
    failures = validate(args)
    if failures:
        print("R2CT_G1_EVIDENCE_VALIDATION=FAIL")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("R2CT_G1_EVIDENCE_VALIDATION=PASS")
    ...
    print(f"predecessor_head={args.predecessor_head}")
    print(f"predecessor_quality_gate=factory_gate_{args.predecessor_quality_gate}")
