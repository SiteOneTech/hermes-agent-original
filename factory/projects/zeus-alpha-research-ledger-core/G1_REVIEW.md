...
**Canonical status reproduced:** Agent Core Factory status still reports `unvalidated_required_docs`. The R2n dispatch snapshot listed 10 blocking required docs and 4 READY rows (`SPRINT_PLAN.md`, `TRACKER.md`, `DOCUMENTATION_INDEX.md`, `QA_GATES.md`) because loose positive review prose in those files could be interpreted as review evidence even while their frontmatter and index entries stayed pending. The strict project-level read-back from primary `repo_path=/home/jean/Projects/hermes-agent-original` shows all 14 required G1 documents `exists=true`, `indexed=true`, `committed=true`, `validated=true`, `reviewed=false`, `readiness_source=primary`. Therefore R2n records both readings, neutralizes the four false-ready sources in branch-local docs, and no normal implementation dispatch is authorized.
