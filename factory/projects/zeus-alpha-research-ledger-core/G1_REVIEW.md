...
**Handoff requirement:** the fresh R2ah branch must be pushed and opened as one non-draft Zeus-signed GitHub PR labeled `agent:zeus` against `main`. The PR body and project-local docs must name the exact head SHA after final push. An independent reviewer must inspect that exact SHA; this R2ah worker does not self-approve, merge, deploy, change credentials, write direct SQL, or touch any runtime/external/trading path.
