...
## Review round 10 — R2y canonical document-status source alignment

**Current status read-back:** R2y starts from current `origin/main` / worktree head `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7` on branch `factory/zeus-alpha-research-ledger-core/inc-001-r2y-repair-canonical-g1-document`. Approved Agent Core Factory status CLI evidence (`/home/jean/Projects/hermes-agent-original/venv/bin/python3 -m hermes_cli.main factory status zeus-alpha-research-ledger-core --json`) was captured in Hermes terminal output `out-1786870710-863987-8590.log`. Its project `document_status` rows show configured base ref `origin/main`, base commit `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7`, `readiness_source=configured_base_ref`, `configured_base_ref_accepted=true`, and `exists=true`, `committed=true`, `validated=true`, `indexed=true`, `reviewed=true`, `blocking=false` for all 14 G1 required documents.

**Correction:** `R2Y_CANONICAL_G1_DOCUMENT_STATUS_SOURCE_ALIGNMENT.md` records that row-level `project.document_status` is now the G1 readiness source to use for the current configured base. Recent reconciler event metadata still naming `unvalidated_required_docs` is stale historical drift from the primary-checkout/PR #20 provenance chain and must not override the live row-level status tuple. The R2y PR must be Zeus-signed, labeled `agent:zeus`, name the exact pushed candidate SHA externally in PR/gate evidence, and receive independent exact-SHA quality review before task closure. This round does not change product/runtime code, merge, deploy, direct SQL, credential state, connector/messaging behavior, or trading/risk/paper/live authority.
