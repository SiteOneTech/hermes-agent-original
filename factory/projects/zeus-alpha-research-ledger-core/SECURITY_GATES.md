...
## R2as canonical Factory CLI source-provenance security gate
- R2as is limited to Factory CLI status-source provenance and project-local evidence docs. It must not add or alter Agent Core ledger runtime/product code, provider clients, credential paths, messaging connectors, deployment behavior, trading/risk/paper/live behavior, external runtimes, primary checkout state, G1 reviewed frontmatter markers, or direct Factory DB writes.
- The exact configured base for this repair is `origin/main` `34a58a6e1c89a66d1e6f177771ba6f9a8cb78af4`, which contains R2ap commit `8e3ac22d7ec0f11d29c9c1938a69a33247bb86ec`. Primary checkout HEAD `4eb87e4cd48105af05fe974cf1d493f0e1b57ae1` is stale/rejected identity evidence only and must not be mutated.
- Security invariant: Factory status must verify running CLI source provenance before backend status readback. If the running source is malformed, unavailable, or otherwise unverifiable, status must fail closed with explicit provenance/error fields and must not substitute stale primary checkout results as canonical G1 readiness.
- Current configured-base G1 rows remain the readiness truth only when the status payload itself carries source-root provenance and per-row configured-base evidence. Historical primary-cwd `reviewed=false` rows, task/event metadata, or source-less status payloads must not dispatch docs-first implementation.
- Delivery remains Zeus-signed `agent:zeus` PR-first plus independent exact-SHA quality review. No self-approval, merge, deploy, credential access, connector/messaging operation, trading/risk/paper/live action, no-auto-merge bypass, direct SQL, primary-checkout mutation, force-push/ref rewrite, or external runtime/product operation is authorized.
