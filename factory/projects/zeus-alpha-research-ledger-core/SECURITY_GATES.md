...
## R2c technical rework current-origin G1 independent review evidence security gate
- R2c is limited to current-origin project-local documentation/index/evidence. It must not add or alter Agent Core ledger runtime/product code, Factory runtime code, provider clients, credential paths, messaging connectors, deployment behavior, trading/risk/paper/live behavior, external runtimes, primary checkout state, or direct Factory DB writes.
- The exact current configured base for this recovery is `origin/main` `b260baea223e863b35fe561e6c5d3d77f3a914c9`. Primary checkout HEAD `4eb87e4cd48105af05fe974cf1d493f0e1b57ae1` is rejected identity evidence only and must not be mutated.
- Historical `unvalidated_required_docs` and `missing_or_unindexed_docs` evidence from R2ai/R2ae/events/gates must remain audit/projection provenance when current required-document rows are non-blocking and active metadata reports `reconciliation_anomalies=[]` from `current_document_status`. It must not be used to self-approve, direct-SQL mutate, clear or recreate blockers, mutate primary checkout, bypass PR-first/no-auto-merge policy, or create a human question for a technical-only cause.
- Delivery is a Zeus-signed `agent:zeus` PR plus read-only exact-SHA review evidence and Factory gate evidence. No merge, deploy, credential access, external connector, messaging operation, trading action, risk mutation, paper/live activation, self-approval, no-auto-merge bypass, direct SQL, primary-checkout mutation, or external runtime execution is authorized.
