...
## R2bd deterministic G1 documentation preflight reconciliation security gate
- R2bd is limited to Factory G1 documentation/status-preflight reconciliation cleanup and project-local evidence docs. It must not add or alter Agent Core ledger runtime/product code, provider clients, credential paths, messaging connectors, deployment behavior, trading/risk/paper/live behavior, external runtimes, primary checkout state, or direct Factory DB writes.
...
- Historical `unvalidated_required_docs` and `missing_or_unindexed_docs` event/task evidence must remain audit history only when current configured-base rows are non-blocking. Resolve-state cleanup may cancel idle resolved-reconciliation backlog, but must not cancel active/claimed/running/review in-flight repair tasks from legacy text; those tasks require explicit evidence, PR handoff, and independent review.
- Delivery is a Zeus-signed `agent:zeus` PR plus independent exact-SHA quality review. No merge, deploy, credential access, external connector, messaging operation, trading action, risk mutation, paper/live activation, self-approval, no-auto-merge bypass, direct SQL, primary-checkout mutation, ALR-020/product dispatch, or external runtime execution is authorized.
