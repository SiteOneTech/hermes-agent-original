...
| R2 | Preserve source reference, timestamps, exact terms/freshness policy, content hash, locator, normalized claim and falsification notes. | contract §2 direct SQL + handler tests |
...
| R4 | Require mechanism, universe, regime/failure regime, data/cost/capacity/no-trade/falsification contract and immutable `research_only`/`unvalidated`/`not_investment_advice` tuple before reviewable state. | contract §3 state/tuple tests |
...
| R7 | Create only inert packages with fixed research-only/not-dispatched state and no recipient/transport/token/URL/action/unknown fields. | contract §3 serialization/negative tests |
| R8 | Provide only JSON handlers in a non-default exact leaf toolset. | default-absence/allowlist tests |
...
| R10 | Use only dedicated Infisical local role secret; no fallback role/DSN. Scheduler stays default-disabled until durable local readiness verifies. | contract §1/§4/§5 tests |
