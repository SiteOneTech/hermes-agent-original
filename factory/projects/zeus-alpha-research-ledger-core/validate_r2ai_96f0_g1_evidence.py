#!/usr/bin/env python3
"""Validate R2ai 96f0 current-origin G1 independent-review evidence (rework).

Read-only validator (no Factory DB writes). It validates a Factory status
JSON captured with the approved CLI from a shared scratch clone of the assigned
branch checked out at exact current origin/main 96f0ecd0a5f17d88a513cf986e5e92edadcbbd40,
plus the committed project-local Markdown pack. It fails if G1 rows are not
clean from the current configured origin/main, if the 14 required document
frontmatter markers lose their independent PR #36/gate 794 provenance, if the
R2ai quality gate is not bound to the exact candidate, or if the residual
...
ARTIFACT = "R2AI_CURRENT_ORIGIN_G1_INDEPENDENT_REVIEW.md"
EXPECTED_SOURCE_ROOT = "/home/jean/.hermes/profiles/quality-reviewer/scratch/r2ai-96f0-g1-review-evidence-repair"
...
CANDIDATE_MARKERS = [
    ...
    "configured_base_ref",
    "stale primary",
    "96f0ecd0a5f17d88a513cf986e5e92edadcbbd40",
]
...
def _validate_repair_docs(project_dir: Path, args: argparse.Namespace, failures: list[str]) -> None:
    ...
    _require("96f0ecd0a5f17d88a513cf986e5e92edadcbbd40" in normalized, f"{ARTIFACT} does not name exact candidate SHA", failures)
    ...
    _require("run-1787263649-3f098a2c" in normalized, f"{ARTIFACT} does not record the rework run id", failures)


def _validate_r2ai_quality_gate(data: dict[str, Any], args: argparse.Namespace, failures: list[str]) -> None:
...
def validate(args: argparse.Namespace) -> list[str]:
    ...
    _require(
        str(data.get("factory_status_source_root") or "").startswith(EXPECTED_SOURCE_ROOT),
        "status factory_status_source_root is not the review source clone",
        failures,
    )
    ...
    _validate_r2ai_quality_gate(data, args, failures)
    ...
    if args.expected_evidence_head == args.expected_base:
        failures.append("expected evidence head equals base; no R2ai commit was validated")
...
def main() -> int:
    ...
    parser.add_argument("--project-dir", default=".", help="Repository root for the assigned worktree/scratch clone")
    parser.add_argument("--status-json", required=True, help="Factory status JSON captured with the approved CLI")
    parser.add_argument("--expected-base", required=True, help="Exact current origin/main base SHA for R2ai")
    parser.add_argument("--expected-evidence-head", required=True, help="Exact R2ai evidence commit SHA (PR head as recorded in gate notes)")
    parser.add_argument("--expected-pr", required=True, help="Exact non-draft R2ai GitHub PR URL")
    parser.add_argument("--expected-quality-gate", required=True, type=int, help="Canonical Factory quality gate id for R2ai")
    args = parser.parse_args()
    failures = validate(args)
    if failures:
        print("R2AI_96F0_G1_EVIDENCE_VALIDATION=FAIL")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("R2AI_96F0_G1_EVIDENCE_VALIDATION=PASS")
