...
## R2af dispatchable required-document validation gate
- R2af must remain documentation/status/reconciliation evidence only under `factory/projects/zeus-alpha-research-ledger-core/`; no runtime/source implementation, deploy, credential, connector, messaging, direct SQL, or trading/risk/paper/live action is authorized.
- Required evidence: stale pre-repair blocker cause for every prompt blocker, current approved Factory CLI `document_status` read-back showing zero G1 required blockers at configured base `origin/main` `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7`, `git diff --check`, scoped diff path verification, and tracked-document verification.
- Delivery evidence must be PR-first: actual GitHub PR against `main`, Zeus signature, `agent:zeus` label, exact candidate SHA named in the PR body, and independent exact-SHA G1/QA review before task closure. This worker must not merge the PR or dispatch downstream ALR implementation.
