...
## R2ag current-base document-status repair gate
- R2ag must remain project-local documentation/reconciliation evidence only under `factory/projects/zeus-alpha-research-ledger-core/`; no product/runtime implementation, deploy, credential, connector, messaging, direct SQL, or trading/risk/paper/live action is authorized.
- Required local evidence: exact current `origin/main` SHA read back from Git, `git diff --check`, scoped diff path verification, tracked-document verification, and approved Factory status CLI read-back showing zero current G1 required-document blockers on configured base ref `origin/main` at `1b6bc0f65d3ad49845d20e056203e3b3702ac2a7`.
- If the Factory payload still lists `metadata.reconciliation_anomalies=["unvalidated_required_docs"]` while the project-level `document_status` has zero blocking G1-required rows, the gate records that exact stale-metadata technical cause and hands off only a bounded control-plane successor task. No generic human question is created.
- Delivery evidence must be PR-first: actual GitHub PR against `main`, Zeus signature, `agent:zeus` label, exact candidate SHA read back from GitHub, and independent exact-SHA `quality-reviewer` verification before task closure. This worker must not merge the PR.
