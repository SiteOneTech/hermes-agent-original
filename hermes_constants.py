...
def _home_env_get(env: dict[str, str] | None, key: str) -> str:
    """Stripped *key* read STRICTLY from *env* when the caller supplied one.

    A supplied env is that caller's whole world (a subprocess env being built, an isolated test), so
    a parent Hermes process must not leak its own ``HOME``/``HERMES_REAL_HOME`` into it. ``None``
    means "this process" and reads ``os.environ``.
    """
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    ``HERMES_REAL_HOME`` is a repair hint for sessions whose ``HOME`` already points at a Hermes
    profile home. A supplied/current ``HOME`` that is a normal OS-user home wins over any inherited
    ``HERMES_REAL_HOME``, so a parent Hermes process cannot leak its own real-home marker into
    isolated tests or explicitly supplied subprocess envs.
    """
    profile_home = _profile_home_path(env)
    explicit = _home_env_get(env, "HERMES_REAL_HOME")
    home = _home_env_get(env, "HOME")
    candidates: list[str] = []
    ...
    candidates.append(_home_env_get(env, "USERPROFILE"))
    drive, path = _home_env_get(env, "HOMEDRIVE"), _home_env_get(env, "HOMEPATH")
    if drive and path:
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    """
    profile_home = _profile_home_path(env)
    mode = _env_get(env or {}, "TERMINAL_HOME_MODE", "auto").lower() or "auto"
    mode = _HOME_MODE_ALIASES.get(mode, mode)

    if mode == "profile":
        return profile_home
    real_home = get_real_home(env)
    current_home = _home_env_get(env, "HOME")
