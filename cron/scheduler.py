...
def _cron_inactivity_seconds() -> float:
    ...
    Shared by the run_job inactivity monitor and the scheduler's fire lifecycle,
    so configured timeout handling remains consistent across execution paths.
    """
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # Workdir is task-scoped, so it is safe on the parallel lane. Per-job
        # profile resolution still changes process-wide config/.env state while
        # it runs, so those jobs retain a serialized lane.
        sequential_jobs = [
            j for j in due_jobs
            if str(j.get("profile") or "").strip()
        ]
        parallel_jobs = [
            j for j in due_jobs
            if not str(j.get("profile") or "").strip()
        ]
        ...
        # Sequential pass for profile jobs. They retain a process-wide profile
        # environment/config override for compatibility with legacy providers.
