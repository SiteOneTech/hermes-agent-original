...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    # file / code-exec commands in the wrong directory.  Jobs with a per-job
    # profile also run in the sequential pool because their profile context
    # temporarily loads profile .env values into process-global os.environ.
    # For workdir-less jobs we leave TERMINAL_CWD untouched — preserves the
    # original behaviour
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # set os.environ["TERMINAL_CWD"] and are additionally protected by
        # run_job's _terminal_cwd_lock so parallel workdir-less jobs cannot
        # observe the override. Profile jobs use a context-local Hermes home
        # override, scheduler _hermes_home hook, and temporary profile .env
        # load into os.environ with snapshot/restore. They MUST run sequentially
        # to avoid corrupting each other. Jobs without either field stay
        # parallel-safe.
        sequential_jobs = [
            j for j in due_jobs
