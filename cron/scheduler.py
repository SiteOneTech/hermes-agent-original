...
def _cron_inactivity_seconds() -> float:
    """Parse HERMES_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600. Shared by the run_job
    inactivity monitor and the scheduler's fire lifecycle, so configured timeout handling stays
...
def _shutdown_parallel_pool() -> None:
    """Shut down every profile's persistent pool, and the sequential pool, on process exit."""
    global _sequential_pool
...
def _restore_environ(snapshot: dict) -> None:
    """Delta-based restore of ``os.environ`` to *snapshot*: drop keys added since, put changed ones
    back. Avoids the brief window where other threads would see an empty env."""
    added = set(os.environ.keys()) - set(snapshot.keys())
    for k in added:
...
def _resolve_job_profile_home(job_id: str, profile: Optional[str]) -> Optional[tuple[str, Path]]:
    """``(normalized_name, home)`` for a job's runtime ``profile``; ``None`` when the job declares
    none, or names one that no longer resolves (fall back to the scheduler's own home).

    Pure resolution — no process/context mutation — so the RUNTIME home is knowable BEFORE anything
    is bound to it. ``_run_one_job_body`` installs the execution secret/terminal scopes outside
    ``run_job`` (they must span delivery too) and so has to resolve the routed profile itself; a
    malformed name still raises out of ``normalize_profile_name``, as it always has.
    """
    ...
    try:
        return normalized_profile, Path(resolve_profile_env(normalized_profile)).resolve()
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    ...
    """
    resolved = _resolve_job_profile_home(job_id, profile)
    if resolved is None:
        yield None
        return
    normalized_profile, profile_home = resolved
...
@contextmanager
def _job_delivery_profile_context(runtime_home: Optional[Path], store_home: Optional[Path] = None):
    """Bind an ALREADY-resolved runtime profile home for the post-run delivery phase, keeping cron
    store writes with the scheduler profile that owns the job. No-op when ``runtime_home`` is None.

    ``run_job`` drops its own ``_job_profile_context`` the moment it returns, so without this the
    FINAL delivery of a ``profile: B`` job resolves its targets, ``config.yaml`` and adapters from
    the SCHEDULER's home — profile A's home channel/tokens send profile B's output. Delivery must
    see B. The job, its outputs and its ledger rows still belong to A, so ``use_cron_store`` pins
    delivery's own bookkeeping write (``last_delivery_unverified``) back to A's store.

    The home is passed in pre-resolved rather than re-derived from ``job['profile']``: the caller
    already resolved it to bind the execution scopes, and delivery must follow the SAME home those
    secrets came from even if the profile directory vanished mid-run.
    """
    ...
    # Resolve the owning store BEFORE the override lands, or it would follow the runtime profile.
    owner_home = Path(store_home) if store_home is not None else _get_hermes_home().resolve()

    global _hermes_home
    prior_override = _hermes_home
    # Same reason ``_job_profile_context`` snapshots: config paths under here still mirror settings
    # into os.environ (e.g. the media-delivery policy), which must not outlive the routed fire.
    env_snapshot = os.environ.copy()
    override_token = None
    try:
        override_token = set_hermes_home_override(runtime_home)
        _hermes_home = runtime_home
        with use_cron_store(owner_home):
            yield
    finally:
        _hermes_home = prior_override
        if override_token is not None:
...
def run_job(
    ...
    """Execute a single cron job under its per-job Hermes profile, if it declares one.

    The profile override is process-wide while it is held (config/.env/HERMES_HOME), which is why
    ``tick()`` dispatches profile jobs on the serialized lane. Jobs without a ``profile`` are
    unaffected — ``_job_profile_context`` yields immediately."""
...
def _save_compose_deliver(
    ...
    ``runtime_home``: the job's runtime profile home (``None`` = the scheduler's own). Saving,
    composing and the incident ledger belong to the OWNING scheduler profile and stay out here;
    only target resolution and the send run under ``runtime_home`` — that is what decides where the
    notice actually goes. ``store_home`` keeps delivery's own cron-store write with the owner."""
    job = d.job
    ...
    if not d.should_deliver:
        return
    # Targets, adapters and delivery config come from the job's RUNTIME profile — the same home its
    # secret/terminal scopes were built from — while the cron-store write stays with the owner.
    with _job_delivery_profile_context(runtime_home, store_home):
        d.unresolved_origin = (
        ...
        try:
            with fence.side_effect_fence() as owns_delivery:
                ...
                d.delivery_error = _deliver_result(
                    ...
                    adapters=adapters,
                    loop=loop,
                    # Failure summaries (and drift/blocked-config alerts composed into
                    # deliver_content on the failure path) honor the job's failure_deliver override
                    # (NS-788).
...
def _deliver_crash_failure(
    ...
    ``runtime_home``/``store_home``: as in ``_save_compose_deliver`` — the crash alert for a
    ``profile: B`` job must leave through B's lane, while the incident ledger stays with the
    owning scheduler profile."""
...
def _run_one_job_body(
    ...
    try:
        ...
        # A job may pin a RUNTIME profile (job['profile']) other than the scheduler profile that owns
        # it. run_job binds that home for the run itself, but the execution scopes below are installed
        # out here because they must also span delivery — so resolve the routed home FIRST and bind
        # them to IT, or a profile-B job executes and delivers with profile-A secrets and terminal
        # policy. Persistence stays with the owner: _store_home is what the job store, the execution
        # ledger and the incident ledger keep using.
        _store_home = _get_hermes_home().resolve()
        try:
            _resolved_profile = _resolve_job_profile_home(job["id"], job.get("profile"))
        except ValueError:
            # Malformed profile name (hand-edited jobs.json): keep the scheduler's own scopes so the
            # failure alert can still resolve credentials, and let run_job raise it loudly as before.
            _resolved_profile = None
        _runtime_home = _resolved_profile[1] if _resolved_profile else None
        _execution_home = _runtime_home or _store_home
        from hermes_constants import get_process_hermes_home

        _process_home = get_process_hermes_home().resolve()
        _execution_home_is_secondary_to_process = (
            hermes_home_key(_execution_home) != hermes_home_key(_process_home)
        )
        # An explicit `profile: default` (or the scheduler's own named profile)
        # resolves to the same home but is not cross-profile routing.  It must
        # retain the ordinary single-profile process-env fallback on a scoped
        # miss.  Use the canonical home key for Windows/case-folding parity.
        _profile_routed_away_from_store = (
            _runtime_home is not None
            and hermes_home_key(_runtime_home) != hermes_home_key(_store_home)
        )

        # The ticker visits every profile store even when adapter multiplexing
        # is off. A store secondary to the process home therefore needs the
        # same private refresh as an explicit runtime-profile route: this outer
        # scope spans run *and* delivery before run_job can load dotenv.
        if (
            is_multiplex_active()
            or _runtime_home is not None
            or _execution_home_is_secondary_to_process
        ):
            from hermes_cli.env_loader import refresh_profile_secret_scope
        ...
        _scope_token = set_secret_scope(_execution_secret_scope)
        ...
        _terminal_scope_token = install_profile_terminal_scope(_execution_home)
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    profile_home = _get_hermes_home().resolve()
    # The host gateway ticks every profile store whether adapter multiplexing is
    # enabled or not. A child for a non-launch store must therefore be scrubbed
    # by its target home, not by the gateway-wide adapter mode. It hydrates that
    # profile's credentials privately after adopting the durable execution.
    from hermes_constants import get_process_hermes_home
    secret_scope_strict = (
        hermes_home_key(profile_home)
        != hermes_home_key(get_process_hermes_home().resolve())
    )
            ...
            json.dump(
                {
                    ...
                    # HERMES_HOME is rewritten in the child, so it cannot
                    # re-derive whether this fire was routed away from the
                    # parent process. Carry the task-local fallback policy.
                    "secret_scope_strict": secret_scope_strict,
                },
                payload_file,
            )
            payload_file.flush()
    ...
    worker_scope_token = set_secret_scope(
        hydrate_and_build_profile_secret_scope(profile_home)
    )
    try:
...
def _run_external_worker_payload(payload_path: Path, ack_path: Path) -> bool:
    ...
    secret_token = set_secret_scope(hydrate_and_build_profile_secret_scope(profile_home))
    strict_token = set_secret_scope_strict(bool(payload.get("secret_scope_strict", False)))
    try:
        with use_cron_store(profile_home):
