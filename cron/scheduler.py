...
def _shutdown_parallel_pool() -> None:
    """Shut down the persistent worker pools on process exit."""
...
def _run_one_job_body(
    ...
    try:
        ...
        _scope_token = set_secret_scope(build_profile_secret_scope(_execution_home))
...
def tick(
    ...
    try:
        ...
        # Workdir is task-scoped, so it is safe on the parallel lane. Per-job profile resolution
        # still mutates process-wide config/.env state while it runs, so those jobs keep a
        # serialized lane.
        sequential_jobs = [j for j in due_jobs if str(j.get("profile") or "").strip()]
        parallel_jobs = [j for j in due_jobs if not str(j.get("profile") or "").strip()]
        ...
        _results: list = []
        _all_futures: list = []
        # Sequential pass for per-job profile jobs. Their profile env/config override is
        # process-wide, so a persistent single-thread pool keeps them from overlapping across ticks
        # WITHOUT blocking the ticker thread; the in-flight guard stops a still-running job from
        # being re-queued on the next tick.
        if sequential_jobs:
            seq_pool = _get_sequential_pool()
            for job in sequential_jobs:
                fut = _submit_with_guard(job, seq_pool, _process_job)
                if fut is None:
                    continue
