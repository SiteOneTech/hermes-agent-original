...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False,
                           caption_media=False, caption=None, mentions=None):
    ...
    ``caption_media`` is the legacy ``tools.send_message_tool._send_whatsapp`` contract: *message* rides as the
    FIRST attachment's caption (any file count) instead of a separate ``/send``. Normal plugin/standalone
    delivery leaves it off and keeps the text-first behavior.
    """
    try:
        import aiohttp
    except ImportError:
    ...
    try:
        ...
        # Legacy shim mode: the text becomes the first attachment's caption instead of its own message.
        text_as_first_caption = bool(caption_media and media and text.strip())
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if text.strip() and not media_caption and not text_as_first_caption:
                payload = _mention_first_payload({"chatId": normalized_chat_id, "message": text})
            ...
            for index, (media_path, is_voice) in enumerate(media):
                # Explicit caption wins; otherwise the shim's text rides on the first file only.
                file_caption = media_caption or (text if text_as_first_caption and index == 0 else None)
                if not os.path.exists(media_path):
                    # In caption mode the words would vanish with the missing file — deliver the caption as a plain message.
                    if file_caption:
                        try:
                            payload = _mention_first_payload({"chatId": normalized_chat_id, "message": file_caption})
                ...
                payload.update({k: v for k, v in (("fileName", os.path.basename(media_path) if media_type == "document" else None), ("caption", file_caption)) if v})
