...
def _adopt_live_compression_child(
    ...
    Resolution uses the canonical transitive ``get_unique_compression_tip`` walk so a lineage with >=2
    compression hops (root -> mid -> tip) recovers to the live tip without selecting between competing
    continuations — the depth-1 ``find_live_compression_child`` lookup this used to call finds no live
    *direct* child in that shape and skipped recovery (#82001). The walk returns the input id when no
    continuation exists and ``None`` when a boundary is ambiguous, and a resolved tip is adopted only
    while its row is still live; every case fails closed.
    """
...
def compress_context(
    ...
    try:
        ...
        # Publishing the compacted transcript stamps fresh SQLite row ids onto the same dicts in
        # place for live reaction surfaces. Those ids are child-session storage identities, not prompt
        # content: returning them would carry a stale persistence marker into the next model turn.
        for message in compressed:
