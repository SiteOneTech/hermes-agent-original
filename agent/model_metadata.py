...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    Separators are normalized away so ``MiniMax-M3``, ``minimax/minimax-m3``, ``minimax_m3`` and
    ``MiniMax M3`` all match; surfaces disagree on the separator and a miss silently reinstates the
    204,800 ``minimax`` catch-all in place of M3's 1M window.
    """
    return "minimaxm3" in re.sub(r"[^a-z0-9]+", "", model.lower())
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"gmi", "commandcode", "commandcode-anthropic"} and base_url:
        ...
        if ctx is not None:
            return ctx
    # 5d. MiniMax's own docs list M3 at 1M context (512K is the max *output*). This curated
    # provider-specific override stays ahead of models.dev, whose catalog exposes that output cap
    # as the context window for some MiniMax entries.
