...
def find_git_worktree(start: str) -> Optional[str]:
    """Return the nearest ancestor dir containing a valid ``.git`` entry, else ``None``.

    A ``.git`` *file* means we're inside a ``git worktree add`` checkout — that counts.
    A ``.git`` *directory* must contain Git's required ``HEAD`` file: accepting an
    arbitrary empty ``.git`` directory would incorrectly make unrelated files belong
    to a workspace.
    """
