...
class LSPClient:
    ...
    async def remove_workspace_folder(self, root: str) -> None:
        """Detach an idle secondary root from a multi-root server.

        The initialization root remains attached for the lifetime of the
        client; it anchors servers that expect ``rootUri`` to remain valid.
        """
        if root == self.workspace_root or root not in self.workspace_folders:
            return
        self.workspace_folders.remove(root)
        await self._send_notification(
            "workspace/didChangeWorkspaceFolders",
            {"event": {"added": [], "removed": [_folder(root)]}},
        )
