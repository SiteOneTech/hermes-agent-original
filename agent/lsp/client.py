...
class LSPClient:
    ...
    async def remove_workspace_folder(self, root: str) -> None:
        ...
        The initialization root is not special-cased here: when its tree is gone, ``rootUri`` is
        already invalid and the folder must go too.  The idle reaper, which retires merely *idle*
        roots, keeps the anchor rule at its own call site instead."""
