...
class LSPService:
    ...
    def __init__(
        ...
        *,
        ...
        # Multi-root clients share one process key, but each attached root has
        # its own activity horizon so an active worktree cannot pin stale
        # sibling worktrees in a server's watcher/index set forever.
        self._root_last_used: Dict[Tuple[str, str], float] = {}
    ...
    def _mark_broken_for_file(self, file_path: str, exc: BaseException) -> None:
        ...
        with self._state_lock:
            ...
            if client is not None:
                for root in client.workspace_folders:
                    self._root_last_used.pop((client.server_id, root), None)
        if client is not None:
            try:
    ...
    async def _get_or_spawn(self, file_path: str) -> Optional[LSPClient]:
        ...
        if client is not None:
            ...
            self._touch(client, root=per_server_root)
            return client
        ...
        try:
            ...
            with self._state_lock:
                ...
                self._root_last_used[(srv.server_id, per_server_root)] = now
            eventlog.log_active(srv.server_id, per_server_root)
    ...
    def _touch(self, client: LSPClient, *, root: Optional[str] = None) -> None:
        """Refresh client activity and, for multi-root servers, root activity.

        ``root`` is resolved before the client cache lookup, so it remains the
        concrete project root even when several roots share one process key.
        """
        now = time.time()
        with self._state_lock:
    ...
    async def _reap_idle_once(self) -> None:
        ...
        with self._state_lock:
            ...
            # A multi-root process can remain hot because one worktree is
            # active while another was deleted or abandoned. Retire only idle
            # secondary roots; the initialization root remains as the stable
            # rootUri anchor for servers that require one.
            stale_roots = []
            for key, client in self._clients.items():
                if key[1] != "":
                    continue
                for root in client.workspace_folders:
                    if root == client.workspace_root:
                        continue
                    root_key = (client.server_id, root)
                    if self._root_last_used.get(root_key, 0) < cutoff:
                        self._root_last_used.pop(root_key, None)
                        stale_roots.append((client, root))
        if stale_roots:
            removals = await asyncio.gather(
                *(client.remove_workspace_folder(root) for client, root in stale_roots),
                return_exceptions=True,
            )
            for (client, root), result in zip(stale_roots, removals):
                if isinstance(result, Exception):
                    logger.debug(
                        "LSP workspace-folder removal failed for %s/%s: %s",
                        client.server_id, root, result,
                    )
        if clients:
            eventlog.log_reaped(
