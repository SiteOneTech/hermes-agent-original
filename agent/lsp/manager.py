...
class LSPService:
    ...
    def __init__(
        ...
        # Multi-root clients share one process key, but each attached root has its own activity
        # horizon so an active worktree cannot pin stale sibling worktrees in a server's
        # watcher/index set forever.
        self._root_last_used: Dict[_Key, float] = {}
    ...
    def _mark_broken_for_file(self, file_path: str, exc: BaseException) -> None:
        ...
        with self._state_lock:
            ...
            if client is not None:
                for attached in client.workspace_folders:
                    self._root_last_used.pop((client.server_id, attached), None)
        if client is not None:
            try:
    ...
    async def _get_or_spawn(self, file_path: str) -> Optional[LSPClient]:
        ...
        with self._state_lock:
            ...
            if client is not None and client.is_running:
                ...
                self._root_last_used[(srv.server_id, root)] = now
    ...
    def _touch(self, client: LSPClient, *, root: Optional[str] = None) -> None:
        """Refresh last-used; guarded on membership so a client reaped mid-operation can't resurrect its
        entry.  ``root`` additionally ages the concrete project root, which multi-root clients share."""
        now = time.time()
        with self._state_lock:
            for key, c in self._clients.items():
                if c is client:
                    self._last_used[key] = now
            if root is not None:
                self._root_last_used[(client.server_id, root)] = now
    ...
    async def _reap_idle_once(self) -> None:
        ...
        with self._state_lock:
            ...
            for reaped in clients:
                for attached in reaped.workspace_folders:
                    self._root_last_used.pop((reaped.server_id, attached), None)

            # A multi-root process can stay hot because one worktree is active while another was
            # deleted or abandoned.  Retire only idle *secondary* roots; the initialization root
            # stays attached as the stable rootUri anchor for servers that require one.
            stale_roots = []
            for key, client in self._clients.items():
                if key[1] != "":  # single-root clients are reaped whole, above
                    continue
                for attached in client.workspace_folders:
                    if attached == client.workspace_root:
                        continue
                    root_key = (client.server_id, attached)
                    if self._root_last_used.get(root_key, 0) < cutoff:
                        self._root_last_used.pop(root_key, None)
                        stale_roots.append((client, attached))
        if stale_roots:
            removals = await asyncio.gather(
                *(client.remove_workspace_folder(attached) for client, attached in stale_roots),
                return_exceptions=True,
            )
            for (client, attached), result in zip(stale_roots, removals):
                if isinstance(result, Exception):
                    logger.debug(
                        "LSP workspace-folder removal failed for %s/%s: %s", client.server_id, attached, result,
                    )
        if clients:
