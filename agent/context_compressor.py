...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
        ...
        # Lean continuity sections are appended after the primary cap above, so re-apply that bound:
        # an unavailable summarizer must not turn the fallback handoff into an oversized transcript
        # copy. Re-inject markers afterward because the second cut may have clipped their block.
        if len(summary) > _FALLBACK_SUMMARY_MAX_CHARS:
            summary = summary[: _FALLBACK_SUMMARY_MAX_CHARS - 42].rstrip() + "\n...[fallback summary truncated]"
            summary = _reinject_pruned_skill_markers(summary, _pruned_names)
        return summary
...
Write only the summary body. Do not include any preamble or prefix."""
    ...
    def _merge_summary_into_tail_row(
        ...
        A failed summarizer's deterministic fallback has its own hard byte budget.  When role
        alternation requires that fallback to share a protected tail row, keeping an unbounded
        tail verbatim would bypass that budget and reproduce the very context pressure the
        fallback is meant to relieve.
        """
        old_content = msg.get("content", "")
        if "Summary generation was unavailable" in summary:
            merge_overhead = len(summary) + len(_SUMMARY_END_MARKER) + 12
            if not (force_user_leading and summary_role == "user"):
                merge_overhead += len(_MERGED_PRIOR_CONTEXT_HEADER) + len(_MERGED_SUMMARY_DELIMITER) + 8
            tail_budget = max(0, _FALLBACK_SUMMARY_MAX_CHARS + 300 - merge_overhead)
            old_text = _content_text_for_contains(old_content)
            if len(old_text) > tail_budget:
                marker = "\n...[protected tail truncated in deterministic fallback]...\n"
                if tail_budget <= len(marker):
                    old_content = marker[:tail_budget]
                else:
                    retained = tail_budget - len(marker)
                    head = retained // 2
                    old_content = old_text[:head] + marker + old_text[-(retained - head):]
