...
def _db_flush_adopt_compression_tip(agent) -> bool:
    ...
    A writer must follow a UNIQUE transitive continuation: picking between live siblings would append the
    user's next turn to the wrong conversation. Read-only consumers keep the best-effort
    ``get_compression_tip`` for sidebar/resume display; this path uses the fail-closed
    ``get_unique_compression_tip`` and refuses to choose when the store cannot provide it."""
    old_id = agent.session_id
    tip = None
    try:
        unique_tip = getattr(type(agent._session_db), "get_unique_compression_tip", None)
        if callable(unique_tip):
            tip = unique_tip(agent._session_db, old_id)
        else:
            logger.warning(
                "compression-safe tip resolver unavailable for %s; refusing to choose a continuation", old_id)
