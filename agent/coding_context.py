...
def _git_root(cwd: Path) -> Optional[Path]:
    current = cwd.resolve()
    # A stray .git under $HOME or a temp dir would otherwise make every scratch path look like a
    # checkout, so those roots never count as a project root.
    ignored_roots: set[Path] = set()
    if (home := _home()) is not None:
        ignored_roots.add(home)
    try:
        temp_root = Path(tempfile.gettempdir()).resolve()
        ignored_roots.update([temp_root, *temp_root.parents])
    except Exception:  # noqa: BLE001 — an unreadable TMPDIR just means no temp exclusions
        pass
    return next(
        (p for p in (current, *current.parents) if p not in ignored_roots and (p / ".git").exists()),
        None,
    )
