...
# tts, image_gen, home-assistant, cron, kanban and computer-use. `todo` rides along with
# canonical `todo_list`: a posture PINS the session toolset (it replaces the platform
# bundle rather than adding to it), so the legacy name every dispatch seam still accepts
# (model_tools._LEGACY_TOOL_ALIASES) has to stay in scope here or an older session/plugin
# that emits it falls out of the posture entirely. Only registered names get a schema, so
# the alias never becomes a second advertised tool.
...
TOOLSETS = {
    ...
    "minimax_cli": _ts(
        "Official MiniMax Token Plan CLI tools for text, image generation, and quota checks",
        ["minimax_cli_status", "minimax_text_chat", "minimax_image_generate"],
    ),
    "video_gen": _ts(
    ...
    # ---- SitioUno/Zeus Agent Core surface (tools live in tools/*_tool.py) ----
    "factory": _ts(
        "SitioUno Software Factory project, lane, task, gate, and status tools",
        ["factory_project_create", "factory_lane_create", "factory_task_create",
         "factory_gate_record", "factory_status"],
    ),
    "agent_management": _ts(
        "Sophie-safe Agent Management onboarding: post-payment intake forms, build reports, "
        "and actuation plans",
    ...
    "agent_management_runtime": _ts(
        "Zeus-only runtime-agent control plane: prepare managed agents, deployment runs, health, "
        "and supervision status",
        ["agent_mgmt_agent_prepare_from_onboarding", "agent_mgmt_runtime_status_update",
         "agent_mgmt_runtime_health_record", "agent_mgmt_agent_status"],
    ),
    "calendar": _ts(
        "Agent-native calendar and booking tools backed by a self-hosted scheduler",
    ...
    "crm": _ts(
        "Agent-native CRM backed by the shared Agent Core DB",
        ["crm_status", "crm_organization_upsert", "crm_contact_upsert",
         "crm_opportunity_upsert", "crm_product_upsert", "crm_quote_create",
         "crm_invoice_create", "crm_relationship_upsert", "crm_interaction_record",
         "crm_follow_up_create", "crm_customer_timeline", "crm_search",
         "crm_twenty_sync", "crm_twenty_raw_request"],
    ),
    "voice": _ts(
        "Agent-native voice/telephony core backed by the shared Agent Core DB, with Vapi as the "
    ...
    "sales": _ts(
        "Agent-native commercial/sales core: catalog, inventory, quotes, orders, invoices, and "
        "payment requests",
    ...
    "sales_operator": _ts(
        "Agent-native daily sales operator: campaigns, territories, source-backed lead research, "
        "supervised outreach, experiments, daily retrospectives, and CRM bridge",
    ...
    "notifications": _ts(
        "Provider-neutral notification adapters for email, marketing, and business notifications",
        ["notification_status", "notification_email_send"],
    ),
    "signature": _ts(
        "Agent-native e-signature core: document requests, signers, signature capture, audit "
        "events, approval hashes, reminders, signed PDFs, final copies, and dashboard metrics",
    ...
    "accounting": _ts(
        "Agent-native accounting-lite core: payment receipts, bank/cash accounts, income/expense "
        "journal, and accountant exports",
    ...
    "fitness": _ts(
        "Agent-native personal fitness coach core: profiles, goals, food database/cache, "
        "nutrition, exercises, routines, workouts, body metrics, check-ins, and progress summaries",
