...
- **Copy API keys from the main profile** — on by default. Each Bot keeps its own profile configuration and static API keys are copied in. Single-use OAuth logins (Anthropic, OpenAI Codex, xAI) are shared from the root credential store rather than copied; sign the Bot in itself with `hermes -p <name> auth add <provider>` when it needs a separate OAuth login. See [cloning profiles and OAuth logins](./profiles.md#clone-everything---clone-all).
