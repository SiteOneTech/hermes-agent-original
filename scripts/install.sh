...
npm_supports_npmrc() {
    ...
    case "$major" in ''|*[!0-9]*) return 1 ;; esac
    case "$minor" in ''|*[!0-9]*) minor=0 ;; esac
    # npm 9 and older cannot satisfy the declared engines.npm lower bound.
    if [ "$major" -lt 10 ]; then
        return 1
    fi
...
check_node() {
    ...
    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        ...
        log_warn "Node.js $(node --version) is too old (Hermes requires Node >=22.22) — installing Hermes-managed Node $NODE_VERSION..."
