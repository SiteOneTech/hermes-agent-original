    ...
    return "\n".join(
        [
            ...
            *[f"- {g.get('gate_type')}: {g.get('status')} reviewer={g.get('reviewer') or '—'}" for g in gates],
            ...
            "- Cada entregable debe quedar en su rama propia. Haz commit local del incremento cuando el scope implique cambios de código/docs; no push/merge/deploy salvo que la tarea lo pida explícitamente y el scope lo permita.",
