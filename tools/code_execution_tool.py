...
def _finish_remote_kernel_result(kernel_result: Dict[str, Any], *,
    ...
    runner_stderr_clipped = bool(kernel_result.get("stderr_clipped"))
    stdout_text = runner_stdout
    stderr_text = runner_stderr
    ...
    if runner_stdout_clipped:
        captured = len(runner_stdout.encode("utf-8", errors="replace"))
        total = max(int(kernel_result.get("stdout_bytes_total", 0) or 0), captured)
        result.update({
        ...
        warnings.append(
            "The remote kernel clipped stdout at its inline limit; only the "
            "captured head is available. Re-run with narrower output if the "
            "omitted data is required."
        )
    if runner_stderr_clipped:
        captured = len(runner_stderr.encode("utf-8", errors="replace"))
        total = max(int(kernel_result.get("stderr_bytes_total", 0) or 0), captured)
        result.update({
            "stderr_truncated": True,
            "stderr_bytes_captured": captured,
            "stderr_bytes_total": total,
            "stderr_bytes_omitted": max(0, total - captured),
        })
        warnings.append(
            "The remote kernel clipped stderr at its inline limit."
        )
    if runner_stdout_clipped or runner_stderr_clipped:
        # A host-side spill here would contain only the runner's already-
        # clipped reply, not the full cell output, so never advertise it as a
        # recovery copy.
    ...
    if result["status"] == "timeout":
        ...
        result["error"] = (
            "Interrupted; the remote session kernel was killed and its state was lost."
        )
