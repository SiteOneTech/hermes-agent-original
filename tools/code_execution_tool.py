...
def _apply_runner_clipping(result: Dict[str, Any], kernel_result: Dict[str, Any],
                           runner_stdout: str, runner_stderr: str) -> None:
    """Fold the remote runner's OWN inline clipping into the result's truncation metadata.

    The runner clips before the host sees the bytes, so host-side counts understate the loss,
    and a host spill would hold only the already-clipped reply — never advertise it as a
    recovery copy."""
    clipped = {"stdout": bool(kernel_result.get("stdout_clipped")),
               "stderr": bool(kernel_result.get("stderr_clipped"))}
    if not any(clipped.values()):
        return
    warnings = [str(result.get("warning") or "").strip()]
    for stream, raw, note in (
        ("stdout", runner_stdout,
         "The remote kernel clipped stdout at its inline limit; only the captured head is "
         "available. Re-run with narrower output if the omitted data is required."),
        ("stderr", runner_stderr, "The remote kernel clipped stderr at its inline limit."),
    ):
        if not clipped[stream]:
            continue
        captured = len(raw.encode("utf-8", errors="replace"))
        total = max(int(kernel_result.get(f"{stream}_bytes_total", 0) or 0), captured)
        result.update({f"{stream}_truncated": True, f"{stream}_bytes_captured": captured,
                       f"{stream}_bytes_total": total,
                       f"{stream}_bytes_omitted": max(0, total - captured)})
...
def _finish_remote_kernel_result(kernel_result: Dict[str, Any], *,
    ...
    runner_stdout = kernel_result.get("stdout", "") or ""
    runner_stderr = kernel_result.get("stderr", "") or ""
    traceback_text = kernel_result.get("traceback", "") or ""
    stdout_text = runner_stdout
    if runner_stderr or traceback_text:
    ...
    _apply_runner_clipping(result, kernel_result, runner_stdout, runner_stderr)
    if result["status"] == "timeout":
        ...
        result["error"] = "Interrupted; the remote session kernel was killed and its state was lost."
