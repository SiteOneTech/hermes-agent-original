...
from agent.skill_utils import EXCLUDED_SKILL_DIRS, SKILL_SUPPORT_DIRS, is_excluded_skill_path
from tools.skills_hub_github import (
    _GIT_OBJECT_SHA_RE, GitHubAuth, GitHubSource, _skip_bundle_file, _tree_members,
)
...
class OptionalSkillSource(SkillSource):
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        if tree is None:
            return None
        if rel not in self._remote_optional_skill_dirs(tree[1]):
            # Revalidate discovery against the LIVE tree before any download, so a stale or
            # locally-poisoned index entry cannot fetch a support package as an installable skill.
            return None
        ...
        if not isinstance(tree_revision, str) or not _GIT_OBJECT_SHA_RE.fullmatch(tree_revision):
            ...
            return None
        files: Dict[str, Union[str, bytes]] = {}
        for rel_file, item_path, regular, blob_sha in _tree_members(
                tree[1], f"{self.OPTIONAL_SKILLS_PREFIX}/{rel}/"):
            if not regular or _skip_bundle_file(rel_file):
                continue
            # By blob id, never by path: the path would float to whatever the branch holds now.
            content = github._fetch_file_bytes(self.OFFICIAL_REPO, item_path, blob_sha=blob_sha)
            if content is None:
        ...
        return self._bundle(rel, files, metadata={
            "source_revision": tree_revision,
            "source_url": f"https://api.github.com/repos/{self.OFFICIAL_REPO}/git/trees/{tree_revision}",
        })

    @staticmethod
    def _remote_optional_skill_dirs(entries: List[dict]) -> Dict[str, bool]:
        """Active optional-skill directories in one Git tree.

        A tree is not a filesystem, so ``is_excluded_skill_path`` cannot tell whether a
        ``references/``-like component belongs to a real skill root. Build the full candidate set
        first, then drop a support subtree only when its parent is itself a skill root — which keeps
        legitimate category paths such as ``scripts/example-skill`` valid."""
        prefix, suffix = f"{OptionalSkillSource.OPTIONAL_SKILLS_PREFIX}/", "/SKILL.md"
        candidates: set = set()
        for item in entries:
            ...
            if not isinstance(path, str) or not path.startswith(prefix) or not path.endswith(suffix):
                continue
            parts = PurePosixPath(path[len(prefix):-len(suffix)]).parts
            if (not parts
                    or any(part in ("", ".", "..") or "\\" in part for part in parts)
                    or any(part.startswith(".") or part in EXCLUDED_SKILL_DIRS for part in parts)):
                continue
            candidates.add("/".join(parts))
        return {rel_dir: True for rel_dir in candidates
                if not any(part in SKILL_SUPPORT_DIRS and idx > 0
                           and "/".join(rel_dir.split("/")[:idx]) in candidates
                           for idx, part in enumerate(rel_dir.split("/")))}
    ...
    def _list_remote_skill_dirs(self) -> Dict[str, bool]:
        ...
        def compute():
            if (tree := self._get_github()._get_repo_tree(self.OFFICIAL_REPO)) is None:
                return None
            return self._remote_optional_skill_dirs(tree[1]) or None

        dirs = _memo_json("official_optional_dirs", compute,
                          valid=lambda c: isinstance(c, dict) and bool(c)) or {}
        # A cached map can predate the support-dir rule (or have been edited on disk), so re-derive
        # it through the same rule instead of trusting what the cache offers.
        self._remote_dirs = self._remote_optional_skill_dirs(
            [{"type": "blob", "path": f"{self.OPTIONAL_SKILLS_PREFIX}/{rel}/SKILL.md"}
             for rel in dirs if isinstance(rel, str)])
        return self._remote_dirs
