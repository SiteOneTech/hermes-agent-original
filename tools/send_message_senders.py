...
async def _registry_standalone_send(platform_name, pconfig, chat_id, message, thread_id=None,
                                    *, media_files=None, force_document=False, **extra):
    """One-shot send through a plugin's ``standalone_sender_fn``. Optional media/caption kwargs are
    forwarded only when set, so senders that take the text-only signature are unaffected."""
