...
def _dispatch_to_plugin_provider_with_name(prompt: str, aspect_ratio: str, provider_name: str):
    """Like ``_dispatch_to_plugin_provider`` but forces a specific provider (the balance-failure
    fallback below). Any failure is ``None`` — the caller keeps the original error."""
    try:
        provider = _get_plugin_provider(provider_name)
    except Exception as exc:
    ...
    if provider is None or not provider.is_available():
        return None
    try:
        result = provider.generate(**_add_provider_kwargs(
            {"prompt": prompt, "aspect_ratio": aspect_ratio}, None, None, None,
            model=_read_configured_image_model()))
    except Exception as exc:
...
# A FAL refusal that names one of these is about the ACCOUNT, not the prompt, so the same
# request can still succeed on another backend.
_BALANCE_FAILURE_MARKERS = ("balance", "credit", "payment", "insufficient", "402", "403")


def _fallback_after_balance_failure(result_json: str, prompt: str, aspect_ratio: str,
                                    *, task_id) -> str:
    """Retry a balance/credit FAL failure through the spare plugin backends; a success, or a
    failure FAL genuinely owns, is returned untouched."""
    try:
    ...
    if not any(marker in err for marker in _BALANCE_FAILURE_MARKERS):
        return result_json
    for provider_name in ("openai", "openai-codex", "xai"):
        fallback = _dispatch_to_plugin_provider_with_name(prompt, aspect_ratio, provider_name)
        if fallback is None:
            continue
        fallback_json = _postprocess_image_generate_result(fallback, task_id=task_id)
        try:
            succeeded = json.loads(fallback_json).get("success")
        except json.JSONDecodeError:
            continue
        if succeeded:
            logger.info("image_generate: FAL failed (%s); fell back to %s",
                        (result.get("error") or "?")[:80], provider_name)
...
def _handle_image_generate(args, **kw):
    ...
    for route in (_dispatch_to_plugin_provider, _maybe_route_managed_krea, image_generate_tool):
        ...
        if raw is not None:
            routed = route
            break
    result_json = _postprocess_image_generate_result(raw, task_id=task_id)
    if routed is not image_generate_tool:
        return result_json
    return _fallback_after_balance_failure(result_json, prompt, aspect_ratio, task_id=task_id)
