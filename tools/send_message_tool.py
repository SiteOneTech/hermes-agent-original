...
# _send_whatsapp itself moved to plugins/platforms/whatsapp/adapter.py::_standalone_send (#41112);
# this shim keeps the pre-migration import path working for tests and the few direct internal
# callers that still do ``from tools.send_message_tool import _send_whatsapp``.
async def _send_whatsapp(extra, chat_id, message, media_files=None, force_document=False):
    """Legacy wrapper for the migrated WhatsApp standalone send.

    Contract preserved from before the plugin migration: the accompanying text rides on the media
    bubble as its caption rather than arriving as a separate message. The plugin exposes that either
    as ``caption_media=True`` or as an explicit ``caption=``; pick whichever this adapter accepts so
    the shim keeps working across plugin revisions.
    """
    media = media_files or []
    caption_kwargs = {"caption_media": True}
    sender, err = _plugin_standalone_sender("whatsapp", label="WhatsApp")
    if err is None:
        try:
            import inspect
            params = inspect.signature(sender).parameters
            if "caption_media" not in params and "caption" in params:
                caption_kwargs = {"caption": message} if (media and message) else {}
    ...
    return await _registry_standalone_send(
        ...
        chat_id,
        "" if "caption" in caption_kwargs else message,
        media_files=media,
        force_document=force_document,
        **caption_kwargs,
    )
