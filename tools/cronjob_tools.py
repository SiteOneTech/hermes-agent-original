...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    try:
        job = create_job_with_scheduler_registration(
            ...
            profile=_normalize_optional_job_value(a["profile"]),
...
def _update_run_fields(job: Dict[str, Any], a: Dict[str, Any], updates: Dict[str, Any]) -> Optional[str]:
    """enabled_toolsets / attach_to_session / workdir / profile / no_agent / repeat / schedule."""
...
_HANDLER_FORWARDED_ARGS = (
    ...
    "script", "context_from", "continuity", "enabled_toolsets", "workdir", "profile", "no_agent",
    "attach_to_session", "paused_reason", "pinned")
