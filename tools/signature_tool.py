...
def _insert_delivery_receipt(
    ...
    *,
    ...
    return sql.statement_one(f"""
      INSERT INTO signature.delivery_receipts (request_id, submitter_id, delivery_type, channel, recipient, provider_message_id, status, error, payload, metadata, delivered_at)
      VALUES ({_q(request_id)}, {_q(submitter.get('submitter_id'))}, 'final_copy', {_q(channel)}, {_q(recipient)}, {_q(provider_message_id)}, {_q(status)}, {_q(error)}, {_j(payload)}, {_j(metadata)}, CASE WHEN {_q(status)} IN ('sent','delivered') THEN now() ELSE NULL END)
      RETURNING *
    """, user=_user())
