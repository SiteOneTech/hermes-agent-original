...
class GitHubSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if tree is None:
            # Keep the legacy path fetch as a degraded-mode fallback when the
            # tree API is unavailable. There is no resolved Git object to pin
            # in this path.
            skill_md = self._fetch_file_content(repo, skill_md_path)
        else:
            _branch, tree_entries = tree
            skill_md_entry = next(
                (
                    item
                    for item in tree_entries
                    if item.get("path") == skill_md_path
                ),
                None,
            )
            # ``tree_data["sha"]`` is a tree-object id, not a ref accepted by
            # the Contents API. Fetch the SKILL.md blob returned by this exact
            # tree instead, just like every companion file below. This pins
            # bytes and file type to the validated tree revision.
            if (
                not isinstance(skill_md_entry, dict)
                or skill_md_entry.get("type") != "blob"
                or skill_md_entry.get("mode") == "120000"
            ):
                return None
            skill_md = self._fetch_file_content(
                repo, skill_md_path, blob_sha=skill_md_entry.get("sha")
            )
        if skill_md is None:
            return None
        ...
        if tree is not None:
            ...
            for item in entries:
                ...
                content = self._fetch_file_bytes(
                    repo, item_path, blob_sha=item.get("sha")
                )
                if content is None:
    ...
    def _fetch_file_bytes(
        ...
        *,
        ...
        ``blob_sha`` uses GitHub's immutable blob endpoint and is preferred
        when a Trees API entry supplies that object id. ``ref`` pins Contents
        API requests to a specific revision when only a path is available.
        Without either, the legacy unpinned Contents API behavior is retained
        for call sites that have no revision in hand.
        """
