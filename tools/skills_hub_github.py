...
class GitHubSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if tree is None:
            ...
            skill_md = self._fetch_file_content(repo, f"{skill_dir}/SKILL.md")
        else:
            ...
            skill_md_sha = _regular_blob_sha(tree[1], f"{skill_dir}/SKILL.md")
            if skill_md_sha is None:
                return None
            skill_md = self._fetch_file_content(repo, f"{skill_dir}/SKILL.md", blob_sha=skill_md_sha)
        if skill_md is None:
            return None
        ...
        if tree is not None:
            if not self._collect_tree_files(repo, skill_dir, tree[1], referenced, files):
                return None
    ...
    def _collect_tree_files(
        ...
        for rel_path, item_path, regular, blob_sha in _tree_members(entries, prefix):
            ...
            try:
                ...
                return False
            self._add_support_file(repo, item_path, rel_path, files, item_path, blob_sha=blob_sha)
