...
# A Git object id as the Trees API reports it (sha-1 today, sha-256 repos tomorrow). Anything
# else must never reach an API path segment.
...
def _tree_members(entries: List[dict], prefix: str):
    """``(rel_path, item_path, is_regular_blob, blob_sha)`` for every git-tree entry under ``prefix``.
    Symlinks (mode 120000) and non-blobs report ``is_regular_blob=False`` so callers can reject a
    SKILL.md-linked symlink instead of silently following it; ``blob_sha`` is the immutable object id
    the entry names, which is what the bytes must be fetched by."""
    for item in entries:
        if not isinstance(item, dict):
            continue
        item_path = item.get("path", "")
        if isinstance(item_path, str) and item_path.startswith(prefix):
            yield (item_path[len(prefix):], item_path,
                   item.get("type") == "blob" and item.get("mode") != "120000", item.get("sha"))


def _regular_blob_sha(entries: List[dict], path: str) -> Optional[str]:
    """Object id of ``path`` when the tree lists it as a REGULAR blob, else None (missing, a
    directory, or a symlink — none of which may be fetched as this skill's file)."""
    for item in entries:
...
class GitHubSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        skill_md_path = _skill_file_path(skill_dir)
        if tree is None:
            # Degraded mode (no tree API answer): nothing resolved to pin to, so this keeps the
            # legacy unpinned Contents fetch rather than refusing the install outright.
            skill_md = self._fetch_file_content(repo, skill_md_path)
        else:
            # The tree's own SHA is a TREE object id, which the Contents API does not accept as a
            # ref; fetch the exact blob this tree names instead, like every support file below.
            skill_md_sha = _regular_blob_sha(tree[1], skill_md_path)
            if skill_md_sha is None:
                return None
            skill_md = self._fetch_file_content(repo, skill_md_path, blob_sha=skill_md_sha)
        if skill_md is None:
            return None
        ...
        if tree is not None:
            complete = self._collect_tree_files(repo, skill_dir, tree[1], referenced, files)
            if complete is None:
                return None
    ...
    def _collect_tree_files(
        self, repo: str, skill_path: str, entries: List[dict], referenced: set,
        ...
        symlinked: set = set()
        complete = True
        for rel_path, item_path, regular, blob_sha in _tree_members(entries, prefix):
            if not regular:
            ...
            try:
                ...
                return None
            complete &= self._add_support_file(repo, item_path, rel_path, files, item_path, blob_sha=blob_sha)
    ...
    def _fetch_file_content(self, repo: str, path: str, ref: Optional[str] = None, *,
                            blob_sha: Optional[str] = None) -> Optional[str]:
        """Fetch a single text file from GitHub (None on miss or non-UTF-8)."""
        content = self._fetch_file_bytes(repo, path, blob_sha=blob_sha, ref=ref)
        try:
            return None if content is None else content.decode("utf-8")
        except UnicodeDecodeError:
            return None

    def _fetch_file_bytes(self, repo: str, path: str, *, blob_sha: Optional[str] = None,
                          ref: Optional[str] = None) -> Optional[bytes]:
        """Fetch exact file bytes.

        ``blob_sha`` reads GitHub's immutable blob endpoint and is what a Trees API entry
        supplies — the bytes and file type can then no longer drift from the tree the paths were
        validated against (see ``fetch`` on the TOCTOU). ``ref`` pins a Contents request when only
        a path is in hand; with neither, the legacy unpinned Contents behavior is kept."""
        if blob_sha is not None:
            if not isinstance(blob_sha, str) or not _GIT_OBJECT_SHA_RE.fullmatch(blob_sha):
                return None
            url, params = f"{_API}/{repo}/git/blobs/{blob_sha}", None
        else:
            url, params = f"{_API}/{repo}/contents/{quote(path, safe='/')}", ({"ref": ref} if ref else None)
