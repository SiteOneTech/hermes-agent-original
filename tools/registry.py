...
def _module_registers_tools(module_path: Path) -> bool:
    """True when the module registers tools at import time — a ``registry.register(...)`` call in the
    module body or inside any module-level control flow (``for``/``if``/``with``/``try``), since
    table-driven modules register a whole family of tools from one loop. Function and class bodies are
    never descended into, so a helper that registers when called does not make its module
    auto-discovered; a text prefilter avoids ``ast.parse`` for files lacking both words."""
    try:
    ...
    def _stmt_registers(stmt: ast.stmt) -> bool:
        ...
        if isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef, ast.Lambda)):
            return False
        return any(
            isinstance(child, ast.stmt) and _stmt_registers(child)
            for child in ast.iter_child_nodes(stmt))

    return any(_stmt_registers(stmt) for stmt in tree.body)
