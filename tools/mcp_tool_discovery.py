...
async def _install_owner_secret_scope():
    ...
    secrets = await asyncio.to_thread(hydrate_and_build_profile_secret_scope, home)
    return set_secret_scope(secrets)
...
def _owner_secret_scope():
    ...
    if home is None:
        ...
        return
    from hermes_cli.env_loader import hydrate_and_build_profile_secret_scope
    token = set_secret_scope(hydrate_and_build_profile_secret_scope(home))
    try:
        yield
    finally:
