...
class SessionCompressionMixin:
    ...
    def get_unique_compression_tip(self, session_id: str) -> Optional[str]:
        ...
        Read-only consumers use :meth:`get_compression_tip` to select the best visible
        descendant when a stale websocket sibling exists.  A stale WRITER must be stricter:
        choosing between two live continuations could append the user's next turn to the wrong
        conversation.  This walk follows a single canonical child at each compression boundary
        and treats multiple live (or compression-ended) candidates as ambiguous.  Closed
        non-compression siblings (e.g. ``ws_orphan_reap``) are ignored — they cannot be a live
        continuation.  Returns the input id when no continuation exists."""
        current = session_id
        ...
        for _ in range(100):
            ...
            current = child_id
        logger.warning(
            "Compression continuation chain from %s exceeded 100 hops; refusing adoption", session_id)
        return None
