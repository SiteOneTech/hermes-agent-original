...
export interface PluginOs {
  ...
  /** Native save dialog. Resolves the chosen CLIENT path, or null on cancel /
   *  when unavailable. It may be handed to a backend only when
   *  `host.state.connectionMode` is `local`. */
  pickSavePath: (options?: PluginFileDialogOptions) => Promise<null | string>
  /** Native open dialog, single CLIENT file. Resolves the chosen path, or null
   *  on cancel / when unavailable. Never pass it to a remote backend. */
