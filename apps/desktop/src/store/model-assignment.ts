...
let cronImpactGeneration = 0
let cronImpactConnectionIdentity = ''
...
function beginCronImpactAssignment(): { connection: string; generation: number } {
  cronImpactGeneration += 1

  return { connection: cronImpactConnectionIdentity, generation: cronImpactGeneration }
}

function currentResponseScope(profile: string, connection: string, generation: number): boolean {
  return (
    profileIdentity() === profile &&
    cronImpactConnectionIdentity === connection &&
    cronImpactGeneration === generation
  )
}

function currentActionScope(profile: string, connection: string): boolean {
  return profileIdentity() === profile && cronImpactConnectionIdentity === connection
}
...
function publishCronImpact(impact: CronModelImpact, profile: string, connection: string): void {
  if (!impact.available) {
    return
  }
...
export function invalidateModelAssignmentCronImpactScope(): void {
  cronImpactGeneration += 1
  dismissNotification(CRON_MODEL_IMPACT_NOTIFICATION_ID)
}
...
export function syncModelAssignmentConnection(connection: HermesConnection | null): void {
  if (!connection) {
    return
  }
  ...
  if (cronImpactConnectionIdentity && cronImpactConnectionIdentity !== next) {
    invalidateModelAssignmentCronImpactScope()
  }

  cronImpactConnectionIdentity = next
}
...
export async function setMainModelAssignment(
  ...
  if (result.cron_model_impact !== undefined) {
    ...
    if (impact) {
      publishCronImpact(impact, profile, connection)
    }
  }
