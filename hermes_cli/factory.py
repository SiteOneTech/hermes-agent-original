...
def factory_command(args: argparse.Namespace) -> int:
    ...
    if command == "project":
        ...
        if sub in {"resume", "pause", "tick", "reconcile", "unblock"}:
...
def add_parser(subparsers) -> argparse.ArgumentParser:
    ...
    for action, help_text in {
        "resume": "Enable autonomous incremental execution for a project",
        "pause": "Pause autonomous execution for a project",
        "tick": "Force one deterministic orchestrator/dispatcher tick",
        "reconcile": "Recompute project/lane status from tasks, runs, gates, and evidence",
        "unblock": "Clear blocked tasks whose effective gate blocker is already resolved",
    }.items():
