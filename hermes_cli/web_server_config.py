...
def _config_schema_with_virtual_fields() -> Dict[str, Dict[str, Any]]:
    """DEFAULT_CONFIG schema plus the virtual fields the normalize/denormalize cycle surfaces:
    ``model_context_length`` right after ``model``, and ``stt.provider`` right after
    ``stt.enabled``. The provider selector is virtual because a command-provider install stores
    ``stt.provider`` outside DEFAULT_CONFIG's walk — without it the dashboard loses the picker
    even though the runtime still honours the setting. Both render adjacent to their owner."""
    ...
    for key, entry in _build_schema_from_config(DEFAULT_CONFIG).items():
        ...
        if key == "stt.enabled":
            ordered["stt.provider"] = {**_SCHEMA_OVERRIDES["stt.provider"], "category": "stt"}
    return ordered
...
def _resolve_assignment_credentials(model_cfg: dict, provider: str, provider_entry: Any,
                                    api_key: str = "") -> None:
    ...
    ``api_key`` is the key the request itself supplied: ``_apply_main_model_assignment`` already
    stored it, so the provider's older stored key must not overwrite it.
    """
    try:
...
def _apply_main_assignment_sync(cfg: dict, provider: str, model: str, base_url: str, api_key: str) -> dict:
    ...
    _resolve_assignment_credentials(model_cfg, provider, provider_entry, api_key)
    cfg["model"] = model_cfg
