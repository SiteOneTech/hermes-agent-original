...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    engine_label, engine_model}`` (empty defaults when missing/unreadable). Never raises — a corrupt
...
def _profile_info(name: str, path: Path, *, is_default: bool, alias_name: Optional[str] = None) -> ProfileInfo:
    ...
    # tools/skills endpoints (and without mutating anything).
    caps = _read_profile_capabilities(path)
    return ProfileInfo(
