...
class ProfileInfo:
    ...
    description: str = ""
    # Optional dashboard-facing display label. This is intentionally separate
    # from ``name``: ``name`` remains the stable on-disk/profile id, while the
    # dashboard can call the root/default profile "Sophie", "Zeus", etc.
    display_name: str = ""
    # Optional dashboard-facing avatar path. Stored as a web path (normally
    # ``/agent-avatars/<asset>.webp``) so the frontend can render a chosen
    # photo without renaming the underlying profile.
    avatar_path: str = ""
    # Optional execution-engine metadata. ``model``/``provider`` describe the
    # Hermes supervisor LLM configured in config.yaml; Factory builder profiles
    # can execute work through an external engine such as Claude Code, Codex CLI,
    # or OpenHands. These fields let dashboards show the measured engine without
    # lying about the supervisor provider.
    engine_label: str = ""
    engine_model: str = ""
...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    Returns empty defaults when the file is missing or unreadable. Never raises
    — a corrupt profile.yaml on an unrelated profile must not break
    ``hermes profile list``.
    """
    path = _profile_yaml_path(profile_dir)
    if not path.is_file():
        return _empty_profile_meta()
    try:
        import yaml
