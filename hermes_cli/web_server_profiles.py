...
def _fallback_profile_entry(profiles_mod, name: str, home: Path, *, is_default: bool,
    ...
    # One read each: the sidebar polls this path, and the visual metadata (display name, avatar)
    # plus the capability rows must match what the ProfileInfo path returns.
    meta = _safe(lambda: profiles_mod.read_profile_meta(home), {})
    # This path runs only when the normal ProfileInfo enumeration failed. Give the
    # caller an accurate recovery view instead of the normal poll-path's possibly cold cache.
    caps = _safe(lambda: profiles_mod._read_profile_capabilities(home), {})
    return {
        ...
        "display_name": meta.get("display_name", ""), "avatar_path": meta.get("avatar_path", ""),
        "engine_label": meta.get("engine_label", ""), "engine_model": meta.get("engine_model", ""),
        "bot_title": meta.get("bot_title", ""),
