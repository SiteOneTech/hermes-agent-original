...
def _runtime_secrets_path():
    """``{HERMES_HOME}/runtime-secrets.env`` — where the gateway writes Infisical-synced secrets."""
    from hermes_cli.doctor import HERMES_HOME
    return HERMES_HOME / "runtime-secrets.env"


def _has_configured_provider_credentials() -> bool:
    """True when provider credentials are reachable, wherever they live.

    Credentials no longer have to sit in ``.env``: an external secret source (Infisical
    runtime-secrets.env, Bitwarden, 1Password) or an Anthropic OAuth login supplies them just as well,
    and reporting "no API key" for a working install sends users back through setup for nothing.
    """
    from hermes_cli.doctor import HERMES_HOME
    env_path = HERMES_HOME / ".env"
    if env_path.exists() and _has_provider_env_config(_read_doctor_env_text(env_path)):
        return True
    from hermes_cli.config import get_env_value
    from hermes_cli.doctor import _PROVIDER_ENV_HINTS
    if any(get_env_value(key) for key in _PROVIDER_ENV_HINTS):
        return True
    with warn_on_error(""):
...
def _check_env_file(should_fix: bool, f: Finding) -> None:
    ...
    if env_path.exists():
        ...
        if _has_configured_provider_credentials():
            # Name the source when the keys came from the Infisical sync rather than .env itself.
            check_ok("API key or custom endpoint configured",
                     "(from Infisical runtime-secrets.env)" if _runtime_secrets_path().exists() else "")
        else:
...
# Optional integrations: keys that unlock a tool rather than a provider, so a missing one warns but is
# never a doctor issue. The source suffix names the secret manager a key came from (see #env_loader).
_INTEGRATION_KEYS: tuple[tuple[str, str], ...] = (
...
@doctor_check()
def _check_integration_api_keys(should_fix: bool, f: Finding) -> None:
    """Per-integration key rows, each labelled with the secret source that supplied it."""
    from hermes_cli.config import get_env_value
    from hermes_cli.doctor import _DHH
    from hermes_cli.env_loader import format_secret_source_suffix
    for label, env_var in _INTEGRATION_KEYS:
        if get_env_value(env_var) or os.environ.get(env_var, ""):
            check_ok(label, f"(configured{format_secret_source_suffix(env_var)})")
        else:
