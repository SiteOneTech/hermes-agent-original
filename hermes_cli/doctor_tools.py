...
def _check_terminal_backend(should_fix: bool, f: Finding) -> None:
    ...
    # In our container docker-in-docker isn't set up, so local is intended: skip the noisy
    # "Docker/Podman not found" warning. Only the local backend is announced this way — an explicitly
    # selected backend (docker with a mounted docker.sock, or a remote one: Vercel, SSH, Daytona) keeps
    # its own check below, and would be mis-described by this line.
