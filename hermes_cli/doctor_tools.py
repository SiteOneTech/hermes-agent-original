...
def _check_terminal_backend(should_fix: bool, f: Finding) -> None:
    ...
    # warning. Only the local backend is announced this way — an explicitly selected backend (docker with a
    # mounted docker.sock, or a remote one: Vercel, SSH, Daytona) keeps its own check below.
