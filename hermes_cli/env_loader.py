...
def load_hermes_dotenv(
    *,
    ...
    # Infisical's materialized runtime file, loaded like the .env files above (override=True) and
    # tagged with its provenance so `hermes doctor` / status lines can name where a key came from.
    ...
    if runtime_env.exists():
        ...
        _load_dotenv_with_fallback(runtime_env, override=True, load_pass=load_pass)
    ...
    # runtime-secrets.env is one of those override loads, so a name an authoritative source owns is
    # reverted here — its provenance goes back with it rather than staying labelled "infisical".
    if _SECRET_SOURCE_RESTORE_BY_HOME:
        for name, value in _SECRET_SOURCE_RESTORE_BY_HOME.get(str(home_path.resolve()), {}).items():
            if os.environ.get(name) != value:
                os.environ[name] = value
                if _SECRET_SOURCES.get(name) == "infisical":
                    del _SECRET_SOURCES[name]
