...
from hermes_cli.web_models import (
    ...
    ProfileSoulUpdate, ProfileDescriptionUpdate, ProfileMetadataUpdate, ProfileModelUpdate,
    ProfileDescribeAuto, SessionPrScanBody)
...
def _profile_to_dict(info) -> Dict[str, Any]:
    ...
    return {
        ...
        "avatar_path": attr("avatar_path", "") or "",
        "engine_label": attr("engine_label", "") or "",
        "engine_model": attr("engine_model", "") or "",
        "bot_title": attr("bot_title", "") or "",
        "toolsets": list(attr("toolsets", []) or []),
        "assigned_skills": list(attr("assigned_skills", []) or []),
        "skill_names": list(attr("skill_names", []) or []),
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # GET /api/profiles is deliberately lazy because it is polled often.  A just-created
    # profile is the one non-poll path where a synchronous inventory is cheap and prevents its
    # first dashboard refresh from falsely showing an empty skill set.
    _best_effort(
        "Warming skill-count cache for new profile %s failed", body.name, default=0,
        fn=lambda: profiles_mod._count_skills(path))
    _best_effort(
        "Warming capability cache for new profile %s failed", body.name, default={},
        fn=lambda: profiles_mod._read_profile_capabilities(path))
...
def _normalize_profile_avatar_path(raw: Optional[str]) -> str:
    """Validate a dashboard avatar path and return the normalized value.

    The dashboard only assigns local static assets under ``/agent-avatars/`` — anything with a
    scheme, host, query, subpath or traversal is a way to point a profile card at a remote (or
    unintended local) resource. Empty clears the assignment and falls back to the
    profile-name-derived default.
    """
    value = (raw or "").strip()
    if not value:
        return ""
    ...
    if parsed.scheme or parsed.netloc or parsed.query or parsed.fragment:
        raise ValueError("avatar_path must be a local /agent-avatars/ asset path")
    if not value.startswith("/agent-avatars/"):
        raise ValueError("avatar_path must start with /agent-avatars/")
    tail = value.removeprefix("/agent-avatars/")
    if not tail or "/" in tail or "\\" in tail or ".." in tail:
        raise ValueError("avatar_path must reference a single local avatar file")
    if not tail.lower().endswith((".webp", ".png", ".jpg", ".jpeg", ".gif")):
        raise ValueError("avatar_path must point to an image file")
    return f"/agent-avatars/{tail}"


@router.put("/api/profiles/{name}/metadata")
async def update_profile_metadata_endpoint(name: str, body: ProfileMetadataUpdate):
    """Update dashboard-facing profile metadata (display name, avatar, description) without
    renaming the profile id. Omitted fields are left untouched."""
    from hermes_cli import profiles as profiles_mod
    profile_dir = _resolve_profile_dir(name)
    ...
    with _profile_errors("PUT /api/profiles/%s/metadata failed", name, not_found=()):
        avatar_path = (_normalize_profile_avatar_path(body.avatar_path)
                       if body.avatar_path is not None else None)
        await run_in_threadpool(
            profiles_mod.write_profile_meta, profile_dir, display_name=display_name,
            avatar_path=avatar_path, description=description,
