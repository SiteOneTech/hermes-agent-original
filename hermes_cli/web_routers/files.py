...
def _fs_find_git_root(start: Path) -> str | None:
    ...
    for _ in range(50):
        try:
            # A worktree/submodule stores .git as a FILE; a real repo dir must hold HEAD. A bare
            # ``mkdir .git`` (or a leftover empty one) is not a repo and would strand the browser
            # at the wrong root.
